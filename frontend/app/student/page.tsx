...
export default function StudentPortal() {
  ...
  const fetchAttendance = async () => {
    try {
      ...
      const res = await apiFetch(
        `/api/reports/student-attendance?userId=${user.userId}`,
      );
      if (res.ok) {
  ...
  const downloadReport = () => {
    const csv =
      "Date,Class,Subject,Status\n" +
      attendance
        .map(
          (r) =>
            `${r.date},${r.class?.name ?? ""},${r.class?.subject ?? ""},${r.status}`,
        )
        .join("\n");
  ...
  const pendingAssignments = assignments.filter(
    (a) =>
      !a.submission && (!a.dueDate || new Date(a.dueDate).getTime() >= now),
  );
  const overdueAssignments = assignments.filter(
    (a) => !a.submission && a.dueDate && new Date(a.dueDate).getTime() < now,
  );
  ...
  const activeExams = exams.filter(
    (e) => e.status === "ACTIVE" && e.attempts.length === 0,
  );
  ...
  const overallGrade = gradedAssignments.length
    ? Math.round(
        gradedAssignments.reduce(
          (sum, a) =>
            sum +
            ((a.submission!.marks ?? 0) / Math.max(1, a.totalMarks)) * 100,
          0,
          ...
          <div className="page-content">
            ...
            <div className="page-body space-y-4">
              ...
              <div className="grid grid-cols-5 gap-2">
                {[
                  ...
                  <Link key={a.href} href={a.href} className="group relative">
                    <div
                      className={`rounded-2xl bg-gradient-to-br ${a.bg} p-3 text-white shadow-sm active:scale-[0.97] transition-transform h-full flex flex-col items-center justify-center gap-1`}
                    >
              ...
              <div className="card p-4">
                ...
                {studentUserId && (
                  <TimetableGrid userId={studentUserId} role="student" />
                )}
              </div>
                ...
                {attendance.length === 0 && (
                  <div className="empty-state py-12">
                    ...
                    <p className="text-sm text-slate-400 dark:text-slate-500 mt-1">
                      Your records will appear here once attendance is taken.
                    </p>
                  </div>
                )}
              </div>
      ...
      <div className="hidden lg:block min-h-screen bg-slate-50 dark:bg-slate-800">
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-teal-600 text-white">
          <div className="max-w-5xl mx-auto px-6 py-8">
            <div className="flex items-center justify-between">
              ...
              <Link
                href="/"
                className="px-3 py-1.5 rounded-lg text-sm bg-white/10 hover:bg-white/20 transition-colors"
              >
                ← Home
              </Link>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4 mt-6">
              <div className="bg-white/10 rounded-xl px-4 py-3 backdrop-blur-sm">
                <p className="text-xs text-emerald-100 uppercase tracking-wider">
                  {t("student.totalRecords")}
                </p>
        ...
        <main className="max-w-5xl mx-auto px-6 py-6 space-y-4">
          ...
          <div className="card p-5">
            <h2 className="text-sm font-bold text-slate-700 dark:text-slate-200 uppercase tracking-wide mb-3">
              📅 My Schedule
            </h2>
            {studentUserId && (
              <TimetableGrid userId={studentUserId} role="student" />
                ...
                {upcomingDeadlines.map((a) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`text-xs font-semibold ${urgency} flex-shrink-0`}
                      >
                        {days <= 0
                          ? "Today"
                          : days === 1
          ...
          {activeExams.length > 0 && (
            <div className="card p-5 border-2 border-purple-200 dark:border-purple-900 bg-purple-50/30">
              <h2 className="text-sm font-bold text-purple-700 dark:text-purple-300 uppercase tracking-wide mb-3">
                  ...
                  >
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-400 dark:text-slate-500">
                        {e.class?.name ?? "General"} · {e.duration} min · Pass{" "}
                        {e.passMark}/{e.totalMarks}
                      </p>
                    </div>
                    <span className="text-xs bg-purple-600 text-white px-3 py-1.5 rounded-lg font-semibold">
                      Start
                    </span>
                  </Link>
                ))}
          ...
          {overallGrade !== null && (
            <div className="card p-5 flex items-center gap-4">
              <div
                className={`w-16 h-16 rounded-2xl flex items-center justify-center text-2xl font-bold text-white ${overallGrade >= 70 ? "bg-emerald-500" : overallGrade >= 50 ? "bg-amber-500" : "bg-red-500"}`}
              >
                {overallGrade}
              </div>
              <div className="flex-1">
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400">
                  {gradedAssignments.length} graded assignment(s) — attendance
                  rate {rate}%
                </p>
              </div>
              <Link
