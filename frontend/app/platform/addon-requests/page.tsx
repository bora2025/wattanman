    ...
    <div className="card p-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 rounded-xl bg-amber-50 flex items-center justify-center text-xl shrink-0">{request.icon || '🧩'}</div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <span className="font-semibold text-slate-800">{request.addonName}</span>
              <span className="text-slate-400">for</span>
              <span className="font-semibold text-slate-800">{request.schoolName}</span>
              <code className="text-[10px] text-slate-400">{request.schoolSubdomain}</code>
            </div>
            <p className="text-xs text-slate-500 mt-1">{priceLabel(request)}</p>
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={dismiss} disabled={dismissing} className="text-xs font-medium text-slate-500 hover:text-red-600 disabled:opacity-50">
    ...
    <div className="page-shell">
      ...
      <div className="page-content">
        ...
        <div className="page-header">
          <h1 className="text-2xl font-bold text-slate-800">Add-on Requests</h1>
          <p className="text-sm text-slate-500 mt-1">Schools asking for a paid add-on, oldest first. Approve from the school's own page once billing is sorted, or dismiss if it's a no.</p>
        </div>
        ...
        <div className="page-body space-y-3">
          ...
          {loading ? (
            ...
            <div className="card p-10 text-center text-slate-400 text-sm">No pending requests right now.</div>
          ) : (
            requests.map(r => (
              <RequestRow
