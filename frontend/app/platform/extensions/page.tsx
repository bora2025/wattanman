...
function VersionPanel({
  ...
  async function upload(file: File) {
    ...
    try {
      ...
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/versions/${version.id}/package`,
          { method: "POST", body },
        ),
      );
      await reload();
  ...
  async function transition(status: string) {
    ...
    try {
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/versions/${version.id}/transition`,
          {
            method: "POST",
  ...
  async function openPreview() {
    ...
    try {
      setPreview(
        await responseJson(
          await apiFetch(
            `/api/platform/extensions/versions/${version.id}/preview`,
          ),
        ),
      );
  ...
  async function appeal() {
    const notes = window.prompt(
      "Explain what changed and why this version should be reviewed again",
    );
    if (!notes) return;
    setBusy(true);
    setError("");
    try {
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/versions/${version.id}/appeal`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  async function deleteVersion() {
    if (
      !window.confirm(
        `Permanently delete draft ${extension.name} v${version.version} and its stored package? This cannot be undone.`,
      )
    )
      return;
    setBusy(true);
    ...
    try {
      await responseJson(
        await apiFetch(`/api/platform/extensions/versions/${version.id}`, {
          method: "DELETE",
        }),
      );
      await reload();
    } catch (deleteError: any) {
      setError(deleteError.message || "Could not delete draft version");
    } finally {
      setBusy(false);
    }
  }
  ...
  > = {
    ...
    PUBLISHED: [
      ...
      { status: "BLOCKED", label: "Emergency block" },
    ],
    DEPRECATED: [
    ...
    BLOCKED: [
      { status: "DEPRECATED", label: "Unblock as deprecated" },
      { status: "RETIRED", label: "Retire" },
    ],
  };
    ...
    <div className="rounded-xl border border-slate-200 dark:border-slate-700 p-4 space-y-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          ...
          <span className="ml-2 text-[11px] rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-1 text-slate-600 dark:text-slate-300">
            {version.lifecycleStatus}
          </span>
          {version.packageChecksum && (
            <p className="text-[10px] text-slate-400 mt-1 font-mono">
              SHA-256 {version.packageChecksum}
            </p>
          )}
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {version.lifecycleStatus === "REJECTED" && (
            <button
              disabled={busy}
              onClick={appeal}
              className="btn-outline btn-sm"
          ...
          {["UPLOADED", "VALIDATED", "REJECTED"].includes(
            version.lifecycleStatus,
          ) && (
            <button
              disabled={busy}
...
function ThemePreview({
  ...
  const [previewMode, setPreviewMode] = useState<"light" | "dark">(
    manifest.mode === "dark" ? "dark" : "light",
  );
  const [surface, setSurface] = useState<"dashboard" | "public">("dashboard");
  const documentHtml = buildThemePreviewDocument(
    manifest,
    css,
    previewMode,
    surface,
  ...
  return (
    <div
      className="fixed inset-0 z-[100] bg-black/60 p-4 flex items-center justify-center"
      role="dialog"
      aria-modal="true"
    >
        ...
        <div className="p-4 flex items-center justify-between border-b border-slate-200 dark:border-slate-700">
          <div>
            ...
            <p className="text-xs text-slate-500">
              Sandboxed iframe; package CSS cannot affect Platform UI.
            </p>
            <p
              className={`text-xs ${preview.platformCompatible ? "text-emerald-600" : "text-red-600"}`}
            >
              Platform {preview.platformVersion} ·{" "}
              {preview.compatibilityRange || "No range"} ·{" "}
              {preview.platformCompatible ? "Compatible" : "Not compatible"}
            </p>
            {preview.warnings?.map((warning) => (
              <p key={warning.code} className="text-xs text-amber-600">
...
function ExtensionCard({
  ...
  const [compatibilityRange, setCompatibilityRange] =
    useState(">=1.0.0 <2.0.0");
  ...
  async function addVersion(event: FormEvent) {
    ...
    try {
      await responseJson(
        await apiFetch(`/api/platform/extensions/${extension.id}/versions`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            version,
  ...
  async function grantPrivateSchool() {
    ...
    try {
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/${extension.id}/private-schools/${schoolId}`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
  ...
  return (
    <article className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:border-indigo-300 hover:shadow-md dark:border-slate-700 dark:bg-slate-900/70 dark:hover:border-indigo-700">
      <div className="h-1.5 bg-gradient-to-r from-indigo-600 via-blue-500 to-cyan-400" />
      <div className="p-5 space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 gap-3">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-indigo-600 to-blue-500 font-bold text-white shadow-sm">{extension.name.slice(0, 2).toUpperCase()}</div>
          <div>
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${extension.visibility === "LISTED" ? "bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300" : extension.visibility === "PRIVATE" ? "bg-violet-100 text-violet-700 dark:bg-violet-950/50 dark:text-violet-300" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"}`}>{extension.visibility}</span>
          </div>
...
function InstallationCard({
  ...
  async function upgrade() {
    ...
    try {
      ...
      const review = await responseJson(
        await apiFetch(
          `/api/platform/extension-installations/${installation.id}/upgrades/${newestVersion.id}/review`,
        ),
      );
  ...
  async function confirmDependencies(versionId: string) {
    const review = await responseJson(
      await apiFetch(
        `/api/platform/extension-installations/${installation.id}/dependencies/${versionId}/review`,
      ),
    );
    const requiredBlockers = (review.dependencies || []).filter(
      (dependency: any) =>
        !dependency.optional && dependency.status !== "SATISFIED",
    );
    if (requiredBlockers.length || review.conflicts?.length) {
      setError(
        [
          ...requiredBlockers.map(
            (dependency: any) => `${dependency.key}: ${dependency.status}`,
          ),
    ...
    const optional = (review.dependencies || []).filter(
      (dependency: any) =>
        dependency.optional && dependency.status !== "SATISFIED",
    );
    return (
      !optional.length ||
      window.confirm(
        `Optional dependencies are unavailable: ${optional.map((dependency: any) => dependency.key).join(", ")}. Continue?`,
      )
    );
  }
  ...
  async function install() {
    try {
      if (!(await confirmDependencies(installation.installedVersion.id)))
        return;
      await action("install", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ versionId: installation.installedVersion.id }),
      });
  ...
  async function submitPilotFeedback() {
    ...
    try {
      const criteria: Array<{ key: string; label: string }> =
        await responseJson(
          await apiFetch(
            "/api/platform/extension-installations/pilot-criteria",
          ),
        );
      ...
      const rating = Number(
        window.prompt(
          "Rate operator confidence from 1 to 5:",
          accepted ? "5" : "3",
        ),
      );
      if (!Number.isInteger(rating) || rating < 1 || rating > 5)
        throw new Error("Pilot rating must be an integer from 1 to 5.");
      const comments =
        window.prompt(
          accepted
            ? "Optional operator comments:"
            : "Describe the operational blocker:",
        ) || "";
      if (!accepted && !comments.trim())
        throw new Error("Comments are required when criteria need work.");
      await responseJson(
        await apiFetch(
          `/api/platform/extension-installations/${installation.id}/pilot-feedback`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              outcome: accepted ? "ACCEPTED" : "NEEDS_WORK",
              rating,
              checklist,
              comments,
            }),
      ...
      setError(
        feedbackError.message || "Could not submit operator pilot feedback",
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900/70 flex items-start justify-between gap-4 flex-wrap">
      <div className="flex gap-3">
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl font-bold text-white ${installation.enabled ? "bg-emerald-500" : "bg-slate-500"}`}>{installation.extension.name.slice(0, 2).toUpperCase()}</div>
        <div>
        <p className="font-semibold text-slate-800 dark:text-slate-100">
          {installation.extension.name}{" "}
          <span className="text-slate-400 font-normal">for</span>{" "}
          {installation.school.name}
        </p>
        <p className="text-xs text-slate-500 dark:text-slate-400">
      ...
      <div className="flex gap-2 flex-wrap">
        ...
        {installation.installedAt &&
          newestVersion &&
          newestVersion.id !== installation.installedVersion.id && (
            <button
              disabled={busy}
              className="btn-outline btn-sm"
              onClick={upgrade}
            >
              Upgrade to v{newestVersion.version}
            </button>
          )}
        {installation.configuration?.rollbackVersionId && (
          <button
            disabled={busy}
            className="btn-outline btn-sm"
        ...
        {installation.installedAt && (
          ...
          >
            {installation.pilotFeedback?.some(
              (feedback) => feedback.source === "OPERATOR",
            )
              ? "Update operator feedback"
              : "Operator feedback"}
          </button>
        )}
        {!installation.uninstalledAt && (
      ...
      {!!installation.pilotFeedback?.length && (
        <div className="basis-full flex gap-2 flex-wrap">
          {installation.pilotFeedback.map((feedback) => (
            <span
              key={feedback.source}
              className="text-[11px] rounded-full bg-slate-100 dark:bg-slate-800 px-2 py-1 text-slate-600 dark:text-slate-300"
            >
              {feedback.source}: {feedback.outcome} · {feedback.rating}/5
            </span>
          ))}
        </div>
      )}
...
function ExtensionsContent() {
  ...
  async function setPublisherStatus(publisherId: string, status: string) {
    if (
      status !== "ACTIVE" &&
      !window.confirm(
        "Suspending or revoking a publisher immediately unlists its extensions and disables active installations. Continue?",
      )
    )
      return;
    try {
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/publishers/${publisherId}/status`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
  ...
  async function registerSigningKey(publisherId: string) {
    const keyId = window.prompt(
      "Signing key ID (for example wattaman-2026-01)",
    );
    if (!keyId) return;
    const publicKeyPem = window.prompt(
      "Paste the Ed25519 public key PEM. Never paste the private key.",
    );
    if (!publicKeyPem) return;
    try {
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/publishers/${publisherId}/signing-keys`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ keyId, publicKeyPem }),
          },
        ),
      );
      await load();
  ...
  async function setSigningKeyStatus(keyId: string, status: string) {
    if (
      status === "REVOKED" &&
      !window.confirm(
        "Revoking a signing key is irreversible and immediately blocks every version signed by it. Continue?",
      )
    )
      return;
    try {
      await responseJson(
        await apiFetch(
          `/api/platform/extensions/signing-keys/${keyId}/status`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ status }),
  ...
  async function createExtension(event: FormEvent) {
    ...
    try {
      await responseJson(
        await apiFetch("/api/platform/extensions", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  async function setAlertStatus(
    ...
    try {
      await responseJson(
        await apiFetch(`/api/platform/extensions/alerts/${alertId}/status`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ status }),
        }),
    ...
    <div className="page-shell">
      ...
      <div className="page-content">
        ...
        <div className="page-header flex flex-wrap items-center justify-between gap-4">
          <div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-indigo-600 dark:text-indigo-400">Platform control center</p><h1 className="mt-1 text-3xl font-bold text-slate-900 dark:text-white">Extensions</h1><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Publish marketplace content, manage school installations, and monitor extension health.</p></div>
          {activeView === "catalog" && <button type="button" className="btn-primary" onClick={() => setShowCreate(value => !value)}>{showCreate ? "Close creator" : "+ New extension"}</button>}
        </div>
        <div className="page-body space-y-5">
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><div className="card p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Catalog</p><p className="mt-2 text-3xl font-bold text-slate-900 dark:text-white">{extensions.length}</p><p className="text-xs text-slate-500">Total extensions</p></div><div className="card p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Published</p><p className="mt-2 text-3xl font-bold text-emerald-600">{extensions.filter(item => item.versions.some(version => version.lifecycleStatus === "PUBLISHED")).length}</p><p className="text-xs text-slate-500">Visible releases</p></div><div className="card p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Active schools</p><p className="mt-2 text-3xl font-bold text-blue-600">{installations.filter(item => item.enabled).length}</p><p className="text-xs text-slate-500">Enabled installations</p></div><div className="card p-4"><p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Needs attention</p><p className="mt-2 text-3xl font-bold text-amber-600">{alerts.filter(item => item.status !== "RESOLVED").length}</p><p className="text-xs text-slate-500">Open operational alerts</p></div></div>
          <div className="sticky top-0 z-20 -mx-1 rounded-2xl border border-slate-200 bg-white/95 p-2 shadow-sm backdrop-blur dark:border-slate-700 dark:bg-slate-900/95">
            <div className="grid grid-cols-3 gap-2">
              {(
                ...
                <button
                  ...
                  className={`rounded-xl px-3 py-3 text-sm font-semibold transition ${activeView === value ? "bg-gradient-to-r from-indigo-600 to-blue-600 text-white shadow-md" : "text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800"}`}
                >
                  {label}{" "}
          ...
          {activeView === "catalog" && (
            <>
              ...
              {showCreate && <form
                ...
                className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-5 shadow-sm dark:border-indigo-900 dark:bg-indigo-950/20 grid md:grid-cols-5 gap-3 items-end"
              >
                <label className="text-xs text-slate-600 dark:text-slate-300">
                  Key
                  <input
              ...
              {loading ? (
                ...
                <div className="space-y-4">{filteredExtensions.map((extension) => (
                  <ExtensionCard
                    key={extension.id}
                    extension={extension}
                    reload={load}
