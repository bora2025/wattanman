      ...
      <div className="page-body space-y-4">
        ...
        {items.map(item => <article key={item.id} className="card p-5">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            {item.status === 'VERIFIED' && <button className="bg-brand-600 hover:bg-brand-700 rounded-lg px-4 py-2 text-sm font-medium text-white" onClick={() => void approve(item)}>Approve</button>}
          </div>
