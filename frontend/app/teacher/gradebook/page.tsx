      ...
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-800 pt-14 lg:pt-0 pb-[72px] lg:pb-0">
        ...
        <main className="flex-1 p-3 sm:p-6 overflow-x-auto">
          <div className="flex items-center justify-between mb-4 sm:mb-6 flex-wrap gap-3">
            <h1 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100">📒 Gradebook</h1>
          ...
          {!selectedClassId ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl p-12 text-center shadow-sm">
              ...
              <p className="text-slate-400 dark:text-slate-500">Select a class to view gradebook</p>
            </div>
          ) : isLoading ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl p-8 text-center shadow-sm">
              ...
              <p className="text-slate-400 dark:text-slate-500 text-sm mt-3">Loading gradebook...</p>
            </div>
          ) : isError ? (
            <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-xl p-6 text-center">
              <p className="text-red-600 dark:text-red-400 mb-2">Failed to load gradebook</p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl p-12 text-center shadow-sm">
              <p className="text-4xl mb-3">📭</p>
              <p className="text-slate-400 dark:text-slate-500">No students in this class</p>
            </div>
          ) : (
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-100 dark:border-slate-800 overflow-auto">
              <table className="min-w-full text-sm">
                <thead className="bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700">
                  <tr>
                    <th className="sticky left-0 bg-slate-50 dark:bg-slate-800 px-4 py-3 text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide min-w-[160px]">Student</th>
                    {gradebook.assignments.map(a => (
                      <th key={a.id} className="px-3 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide min-w-[100px]">
                        <p className="truncate max-w-[96px]" title={a.title}>{a.title}</p>
                        <p className="text-slate-400 dark:text-slate-500 font-normal normal-case">/{a.totalMarks}</p>
                      </th>
                    ))}
                    <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide min-w-[80px]">Avg</th>
                    <th className="px-4 py-3 text-center text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide min-w-[80px]">Grade</th>
                  </tr>
                </thead>
                <tbody>
                  {gradebook.rows.map((row, i) => {
                    ...
                    return (
                      <tr key={row.student.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                        <td className="sticky left-0 bg-inherit px-4 py-3 font-medium text-slate-800 dark:text-slate-100 border-r border-slate-100 dark:border-slate-800">
                          {row.student.user.name}
                        </td>
                        {gradebook.assignments.map(a => {
                          const sub = row.submissions[a.id]
                          if (!sub) return <td key={a.id} className="px-3 py-3 text-center text-slate-300 text-xs">—</td>
                          if (sub.marks === null) return <td key={a.id} className="px-3 py-3 text-center"><span className="text-xs bg-amber-100 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 px-1.5 py-0.5 rounded-full">Pending</span></td>
              ...
              {/* Legend */}
              <div className="px-4 py-3 border-t border-slate-100 dark:border-slate-800 flex items-center gap-4 text-xs text-slate-500 dark:text-slate-400">
                ...
                <Link href="/teacher/assignments" className="ml-auto text-sky-600 dark:text-sky-400 hover:underline">Manage Assignments →</Link>
              </div>
            </div>
          )}
        </main>
