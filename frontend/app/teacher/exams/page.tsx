          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500 dark:text-slate-400 mt-0.5">Author exams and grade student attempts</p>
              </div>
            ...
            {isLoading ? (
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl border border-gray-100 dark:border-slate-800 shadow-sm">
                <EmptyState icon="📝" message="No exams created yet" action={
                  <Link href="/teacher/exams/new?returnTo=/teacher/exams" className="text-sm text-sky-600 dark:text-sky-400 font-medium hover:underline">Create your first exam</Link>
                } />
              </div>
            ) : (
                ...
                {exams.map(exam => (
                  <div key={exam.id} className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm p-4 border border-gray-100 dark:border-slate-800 flex items-center justify-between gap-4 hover:shadow-md transition-shadow">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-amber-600 dark:text-amber-400 mt-0.5">{exam._count.attempts} attempt(s)</p>
                      <p className="text-xs text-gray-400 mt-0.5">{STATUS_HINT[exam.status]}</p>
                    </div>
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Link href={`/teacher/exams/${exam.id}/edit?returnTo=/teacher/exams`} className="text-xs text-sky-600 dark:text-sky-400 hover:underline font-medium">
                        Edit
                      </Link>
                      <Link
                        href={`/teacher/exams/${exam.id}/attempts`}
                        className="text-xs px-2.5 py-1.5 rounded-lg border border-sky-200 dark:border-sky-900 text-sky-700 dark:text-sky-300 hover:bg-sky-50 dark:hover:bg-sky-950/40 font-medium"
                      >
                      ...
                      <select value={exam.status}
                        ...
                        className="text-xs border border-gray-200 dark:border-slate-700 rounded-lg px-2 py-1.5 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-300">
