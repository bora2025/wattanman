          ...
          <div className="flex-1 overflow-y-auto">
            {showNewList && (
              <div className="border-b border-slate-100 pb-2">
                {children.length > 0 && (
                  <>
                    <p className="px-4 pt-3 pb-1 text-[10px] uppercase tracking-wide text-slate-400">My children</p>
                    {children.map(c => (
                      <button key={c.id} onClick={() => { setSelectedPartnerId(c.id); setShowNewList(false) }}
                        ...
                        <span className="w-7 h-7 rounded-full bg-emerald-100 text-emerald-700 text-xs font-bold flex items-center justify-center">{c.name.charAt(0).toUpperCase()}</span>
                        <span>{c.name} <span className="text-slate-400 text-xs">(Student)</span></span>
                      </button>
                    ))}
                  </>
                )}
