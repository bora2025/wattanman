...
const LeafletMap = dynamic(() => import('./LeafletBusMap'), { ssr: false, loading: () => (
  <div className="w-full h-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center rounded-xl">
    <p className="text-slate-400 dark:text-slate-500 text-sm">Loading map...</p>
  </div>
) })
      ...
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-800 pb-[72px] lg:pb-0">
        ...
        <aside className={`${selectedBusId ? 'hidden lg:flex' : 'flex'} w-full lg:w-60 bg-white border-r border-slate-200 flex-col`}>
          <div className="p-4 border-b border-slate-100 dark:border-slate-800">
            <Link href="/parent" className="text-xs text-emerald-600 dark:text-emerald-400">← Back</Link>
            <p className="text-base font-bold text-slate-800 dark:text-slate-100 mt-2">🚌 School Bus Tracker</p>
            <p className="text-xs text-slate-400 dark:text-slate-500">Live GPS tracking</p>
          </div>

          <div className="flex-1 overflow-y-auto p-3">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">Select Bus</p>
            {buses.length === 0 ? (
              <p className="text-xs text-slate-400 dark:text-slate-500 text-center py-8">No buses registered</p>
            ) : (
              buses.map(bus => {
                ...
                return (
                  <button key={bus.id} onClick={() => setSelectedBusId(bus.id)}
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{bus.licensePlate}</p>
                      <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${meta.color}`}>{meta.label}</span>
                    </div>
                    <p className="text-xs text-slate-500 dark:text-slate-400">{bus.driverName ?? 'No driver assigned'}</p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">Capacity: {bus.capacity}</p>
                  </button>
                )
              })
            )}
          ...
          {!selectedBusId ? (
            <div className="flex-1 flex items-center justify-center bg-slate-50 dark:bg-slate-800">
              <div className="text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 font-medium">Select a bus to track</p>
                <p className="text-slate-400 dark:text-slate-500 text-sm mt-1">Location updates every 10 seconds</p>
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 px-4 sm:px-6 py-3 flex items-center justify-between gap-2 sticky top-0 z-10">
                <button onClick={() => setSelectedBusId(null)} className="lg:hidden w-9 h-9 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 active:scale-95 transition-transform shrink-0" aria-label="Back">←</button>
                <div className="flex-1 min-w-0">
                  <p className="font-bold text-slate-800 dark:text-slate-100 truncate">Bus {selectedBus?.licensePlate}</p>
                ...
                {location?.speed !== null && location?.speed !== undefined && (
                  <div className="text-right shrink-0">
                    <p className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100">{Math.round(location.speed)}</p>
                  ...
                  {location ? (
                    ...
                    <div className="w-full h-full bg-slate-100 dark:bg-slate-800 flex flex-col items-center justify-center rounded-xl">
                      {locationLoading ? (
                        ...
                        <p className="text-slate-400 dark:text-slate-500 text-sm">Fetching GPS location...</p></>
                      ) : (
                        <><p className="text-4xl mb-3">📍</p>
                        <p className="text-slate-400 dark:text-slate-500">No location data available for this bus</p></>
                      )}
                    </div>
                  )}
                </div>
