    ...
    <AuthGuard requiredRole="PARENT">
      <div className="flex min-h-screen bg-slate-50 dark:bg-slate-800 pb-[72px] lg:pb-0">
        ...
        <main className="flex-1 p-4 sm:p-6 max-w-3xl mx-auto w-full">
          <h1 className="text-xl sm:text-2xl font-bold text-slate-800 dark:text-slate-100 mb-4">📊 Grades & Progress</h1>
          ...
          {avg !== null && (
            <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm p-4 mb-6 flex items-center gap-4 border border-slate-100 dark:border-slate-800">
              ...
              <div>
                <p className="font-semibold text-slate-800 dark:text-slate-100">Overall Average</p>
                <p className="text-sm text-slate-500 dark:text-slate-400">{grades.length} graded assignment(s)</p>
              </div>
            </div>
          )}

          {!childId ? (
            <p className="text-slate-400 dark:text-slate-500 text-center py-12">Select a child to view grades</p>
          ) : isLoading ? (
            ...
            <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-xl p-4 text-center">
              <p className="text-red-600 dark:text-red-400 text-sm mb-1">Failed to load grades</p>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-xl p-12 text-center shadow-sm">
              <p className="text-slate-400 dark:text-slate-500">No grades recorded yet</p>
            </div>
          ) : (
            <div className="space-y-3">
              {grades.map(g => {
                ...
                return (
                  <div key={g.id} className="bg-white dark:bg-slate-900 rounded-xl shadow-sm p-4 border border-slate-100 dark:border-slate-800">
                    <div className="flex items-center justify-between mb-2">
                      <div>
                        <p className="font-semibold text-slate-800 dark:text-slate-100">{g.assignment.title}</p>
                        <p className="text-xs text-slate-400 dark:text-slate-500">{g.assignment.class.name} · {g.assignment.class.subject}</p>
                      </div>
                      <div className="text-right">
                        <p className="font-bold text-lg text-slate-800 dark:text-slate-100">{g.marks}<span className="text-sm text-slate-400 dark:text-slate-500">/{g.assignment.totalMarks}</span></p>
                    ...
                    {g.feedback && <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 italic">"{g.feedback}"</p>}
                  </div>
                )
              })}
            </div>
