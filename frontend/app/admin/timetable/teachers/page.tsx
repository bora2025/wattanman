                      ...
                      <div className="overflow-hidden rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900">
                        <table className="w-full text-sm">
                          <thead><tr className="bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
                            ...
                            <th className="px-4 py-3 text-left font-semibold text-gray-600 dark:text-slate-300">Contact</th>
                            <th className="px-4 py-3 text-left font-semibold text-gray-600 dark:text-slate-300">Class Teacher</th>
                            <th className="px-4 py-3 text-center font-semibold text-gray-600 dark:text-slate-300">Lessons</th>
                              ...
                              <tr key={t.id}
                                ...
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-2">
                                    {t.photo ? (
                                      <img src={t.photo} alt={t.firstName} className="w-7 h-7 rounded-full object-cover flex-shrink-0 border border-gray-200 dark:border-slate-700" onError={e => { e.currentTarget.style.display = 'none' }} />
                                    ) : (
                                    ...
                                    <span className="font-medium text-gray-800 dark:text-slate-100">{[t.firstName, t.lastName].filter(Boolean).join(' ')}</span>
                                  </div>
                                </td>
                                <td className="px-4 py-3">
                                ...
                                <td className="px-4 py-3 text-right" onClick={e => e.stopPropagation()}>
                                  <button onClick={() => openTeacherModal(t)} disabled={!partTimeTeacherEnabled} className="text-blue-600 dark:text-blue-400 hover:underline text-sm mr-2 disabled:opacity-40 disabled:pointer-events-none">Edit</button>
                                  <button onClick={() => { setSelectedTeacher(t); openLessonModal(undefined, t.id) }} className="text-emerald-600 dark:text-emerald-400 hover:underline text-sm mr-2">+ Lesson</button>
                                  <button onClick={() => setDeleteTeacherId(t.id)} disabled={!partTimeTeacherEnabled} className="text-red-500 dark:text-red-400 hover:underline text-sm disabled:opacity-40 disabled:pointer-events-none">Remove</button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                      ...
                      {!selectedTeacher ? (
                        ...
                        <div className="space-y-2">
                          {teacherLessons.map(l => (
                            <div key={l.id} className="border border-gray-200 dark:border-slate-700 rounded-lg p-3">
                              <div className="flex items-start justify-between gap-2">
                                <div className="space-y-1 flex-1 min-w-0">
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    ...
                                    <span className="text-sm text-gray-800 dark:text-slate-100 truncate">{l.subject.name}</span>
                                  </div>
                                  <div className="flex items-center gap-1.5 flex-wrap">
                                    <span className="rounded px-1.5 py-0.5 text-[10px] font-semibold text-white flex-shrink-0"
                                ...
                                <div className="flex flex-col gap-1 flex-shrink-0">
                                  <button onClick={() => openLessonModal(l)}
                                    className="text-xs text-blue-600 dark:text-blue-400 hover:underline text-right">Edit</button>
                                  <button onClick={() => setDeleteLessonId(l.id)}
                                    className="text-xs text-red-500 dark:text-red-400 hover:underline text-right">Remove</button>
                                </div>
                              </div>
                            </div>
                          ))}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
              <h2 className="font-bold text-gray-800 dark:text-slate-100">{editingTeacher ? 'Edit Teacher' : 'New Teacher'}</h2>
              <button onClick={() => setShowTeacherModal(false)} className="text-gray-400 hover:text-gray-600 text-xl">×</button>
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
              {!editingTeacher && (
                <div>
                  <label className="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">From existing teachers</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">Sex</label>
      ...
      {showLessonModal && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-md">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
              <h2 className="font-bold text-gray-800 dark:text-slate-100">{editingLesson ? 'Edit Lesson' : 'New Lesson'}</h2>
            ...
            <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 flex justify-end gap-2">
              <button onClick={() => setShowLessonModal(false)} className="px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-lg text-sm font-medium">Cancel</button>
      ...
      {deleteTeacherId && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-sm p-6 text-center">
            <p className="text-gray-800 dark:text-slate-100 font-semibold mb-4">Remove this teacher? Their lesson contracts will also be removed.</p>
            <div className="flex justify-center gap-3">
              <button onClick={() => setDeleteTeacherId(null)} className="px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-lg text-sm">Cancel</button>
      ...
      {deleteLessonId && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <div className="flex justify-center gap-3">
              <button onClick={() => setDeleteLessonId(null)} className="px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-lg text-sm">Cancel</button>
