...
interface TClass {
  ...
  short: string;
  color: string | null;
  printSubjectPicture: boolean;
  customFields: any;
  classTeachers?: TTeacher[];
}
...
function getPeriodTimes(tt: {
  ...
  if (tt.periodTimes) {
    try {
      ...
      const result = [...saved];
      while (result.length < tt.periodsPerDay)
        result.push(defaults[result.length] ?? "00:00");
  ...
  return (
    <div className="space-y-1.5">
      {rules.map((rule, i) => (
        <div
          key={i}
          className="flex items-center gap-1.5 bg-gray-50 dark:bg-slate-800 rounded-lg px-2 py-1.5"
        >
          ...
          <button
            ...
            className="ml-auto text-red-400 hover:text-red-600 dark:hover:text-red-400 font-bold text-sm leading-none"
          >
            ×
          </button>
        </div>
      ...
      <button
        type="button"
        onClick={() => onChange([...rules, { from: "12:00", to: "13:00" }])}
...
export default function TimetablePage() {
  ...
  const WIZARD_STEPS = [
    t("timetable.schoolInfo"),
    t("timetable.subjects"),
    t("timetable.classes"),
    t("timetable.classrooms"),
    t("timetable.teachersContracts"),
  ];
  ...
  const partTimeTeacherEnabled =
    enabledModules === null || enabledModules.includes("PART_TIME_TEACHER");
  useEffect(() => {
  ...
  const [wizardTimetableId, setWizardTimetableId] = useState<string | null>(
    null,
  );
  ...
  const [wizardSystemClasses, setWizardSystemClasses] = useState<
    {
      id: string;
      name: string;
  ...
  >([]);
  const [wizardSystemClassesLoading, setWizardSystemClassesLoading] =
    useState(false);
  ...
  const [printSigners, setPrintSigners] = useState<string[]>([
    "Teacher",
    "Admin",
  ]);
  ...
  const [workloadSort, setWorkloadSort] = useState<"name" | "week" | "month">(
    "week",
  );
  ...
  function openColorPicker(current: string): Promise<string> {
    ...
    return new Promise((resolve) => {
      colorPickerResolve.current = resolve;
    });
  }
  ...
  function openWizard() {
    ...
    setWizardError("");
    setWSchoolName("");
    setWAcademicYear("2025-2026");
    setWPeriods(8);
    setWDays(5);
    setWWeekend(["SATURDAY", "SUNDAY"]);
    setWTimeOffRules([]);
    setWDistrib("");
    setWHomework("");
    setWMaxOn("");
    ...
    if (wizardStep === 0) {
      ...
      const res = await apiFetch("/api/timetable", {
        ...
        body: JSON.stringify({
          ...
          numberOfDays: wDays,
          weekend: wWeekend,
          timeOffRules: serializeTimeOffRules(wTimeOffRules),
      ...
      if (res.ok) {
        ...
        const msg =
          body?.message ?? `Error ${res.status} — could not create timetable.`;
        setWizardError(msg);
        showToast(msg, false);
        setWizardSaving(false);
      }
  ...
  async function saveSubject() {
    ...
    if (res.ok) {
      showToast(
        isEdit
          ? `Subject "${fSubName}" updated.`
          : `Subject "${fSubName}" added.`,
      );
      await loadTimetable(id);
      setShowSubjectModal(false);
    } else {
      showToast(`Failed to ${isEdit ? "update" : "add"} subject.`, false);
    }
  }
  async function removeSubject(subId: string, name: string) {
    const id = ttId();
    if (!id) return;
    const res = await apiFetch(`/api/timetable/subjects/${subId}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  async function saveClass() {
    ...
    const isEdit = !!editingItem;
    const url = isEdit
      ? `/api/timetable/classes/${editingItem.id}`
      : `/api/timetable/${id}/classes`;
    const res = await apiFetch(url, {
      method: isEdit ? "PUT" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        ...
        short: fClsShort,
        color: fClsColor,
        printSubjectPicture: fClsPrint,
        customFields: fClsCustom ? { note: fClsCustom } : undefined,
      }),
    });
    if (res.ok) {
      showToast(
        isEdit ? `Class "${fClsName}" updated.` : `Class "${fClsName}" added.`,
      );
      await loadTimetable(id);
      setShowClassModal(false);
    } else {
      showToast(`Failed to ${isEdit ? "update" : "add"} class.`, false);
    }
  }
  async function removeClass(clsId: string, name: string) {
    const id = ttId();
    if (!id) return;
    const res = await apiFetch(`/api/timetable/classes/${clsId}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  async function saveClassroom() {
    ...
    const isEdit = !!editingItem;
    const url = isEdit
      ? `/api/timetable/classrooms/${editingItem.id}`
      : `/api/timetable/${id}/classrooms`;
    const res = await apiFetch(url, {
      method: isEdit ? "PUT" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (res.ok) {
      showToast(
        isEdit
          ? `Classroom "${fRmName}" updated.`
          : `Classroom "${fRmName}" added.`,
      );
      await loadTimetable(id);
      setShowClassroomModal(false);
    } else {
      showToast(`Failed to ${isEdit ? "update" : "add"} classroom.`, false);
    }
  }
  async function removeClassroom(rmId: string, name: string) {
    const id = ttId();
    if (!id) return;
    const res = await apiFetch(`/api/timetable/classrooms/${rmId}`, {
      method: "DELETE",
    });
    if (res.ok) {
      showToast(`Classroom "${name}" removed.`);
      await loadTimetable(id);
  ...
  async function saveTeacher() {
    ...
    const isEdit = !!editingItem;
    const url = isEdit
      ? `/api/timetable/teachers/${editingItem.id}`
      : `/api/timetable/${id}/teachers`;
    const res = await apiFetch(url, {
      method: isEdit ? "PUT" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        lastName: fTLast,
        firstName: fTFirst,
        khmerName: fTKhmer || null,
        short: fTShort,
        sex: fTSex || null,
        email: fTEmail || null,
        phone: fTPhone || null,
        color: fTColor,
        classTeacherId: fTClassTeacher || null,
      }),
    });
    ...
    if (res.ok) {
      showToast(
        isEdit
          ...
          : `Teacher "${fullName}" added.`,
      );
      await loadTimetable(id);
      setShowTeacherModal(false);
    } else {
      const err = await res.json().catch(() => ({}));
      const reason = Array.isArray(err?.message)
        ? err.message[0]
        : (err?.message ?? `HTTP ${res.status}`);
      showToast(
        `Failed to ${isEdit ? "update" : "add"} teacher: ${reason}`,
        false,
      );
    }
  }
  async function removeTeacher(tchId: string, name: string) {
    const id = ttId();
    if (!id) return;
    const res = await apiFetch(`/api/timetable/teachers/${tchId}`, {
      method: "DELETE",
    });
    if (res.ok) {
  ...
  async function saveLesson() {
    ...
    const isEdit = !!editingItem;
    const url = isEdit
      ? `/api/timetable/lessons/${editingItem.id}`
      : `/api/timetable/${id}/lessons`;
    const res = await apiFetch(url, {
      method: isEdit ? "PUT" : "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (res.ok) {
      ...
      showToast(`Failed to ${isEdit ? "update" : "add"} lesson.`, false);
    }
  }
  async function removeLesson(lsnId: string) {
    const id = ttId();
    if (!id) return;
    const res = await apiFetch(`/api/timetable/lessons/${lsnId}`, {
      method: "DELETE",
    });
    if (res.ok) {
      showToast("Lesson contract removed.");
      await loadTimetable(id);
  ...
  async function handleGenerate() {
    ...
    if (!id) return;
    setGenerating(true);
    const res = await apiFetch(`/api/timetable/${id}/generate`, {
      method: "POST",
    });
    if (res.ok) {
      ...
      showToast(t("timetable.generateFailed"), false);
    }
    setGenerating(false);
  }
  ...
  async function saveSettings() {
    ...
    const res = await apiFetch(`/api/timetable/${current.id}`, {
      ...
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        periodsPerDay: sPeriods,
        numberOfDays: sDays,
        weekend: sWeekend,
        timeOffRules: serializeTimeOffRules(sTimeOffRules),
      }),
    });
    if (res.ok) {
  ...
  async function handleDeleteTimetable(
    ...
    if (
      !window.confirm(
        `Delete "${name}"?\n\nThis will permanently remove the timetable and all its subjects, classes, classrooms, teachers, lessons and entries.`,
      )
    )
      return;
    ...
    if (res.ok) {
      ...
      showToast(
        typeof reason === "string"
          ? reason
          : (reason[0] ?? `Failed to delete "${name}".`),
        false,
      );
    }
  }
  ...
  async function placeEntry(
    ...
    if (lesson.id) {
      const placed = current.entries.filter(
        (e) => e.lessonId === lesson.id && e.id !== sourceEntryId,
      ).length;
      if (placed >= lesson.perWeek) {
        showToast(
          `Fully scheduled: ${lesson.subject.short} for ${lesson.class.short} already placed ${placed}/${lesson.perWeek}×/week`,
          false,
        );
        return false;
      }
    ...
    if (conflict) {
      showToast(
        `Conflict: ${lesson.teacher.short} already has ${conflict.subject.short} (${conflict.class.short}) at this slot`,
        false,
      );
      return false;
    }
  ...
  async function removeEntry(entry: TEntry) {
    const res = await apiFetch(`/api/timetable/entries/${entry.id}`, {
      method: "DELETE",
    });
    if (res.ok) {
      setCurrent((prev) =>
        prev
          ? { ...prev, entries: prev.entries.filter((e) => e.id !== entry.id) }
          : prev,
      );
    } else {
  ...
  function openSubjectModal(item?: TSubject) {
    ...
    setFSubName(item?.name ?? "");
    setFSubShort(item?.short ?? "");
    setFSubColor(item?.color ?? "#6366f1");
    setFSubRooms(item?.classroomCount ?? 1);
    setFSubCustom(item?.customFields?.note ?? "");
    setShowSubjectModal(true);
  }
  function openClassModal(item?: TClass) {
    setEditingItem(item ?? null);
    setFClsName(item?.name ?? "");
    setFClsShort(item?.short ?? "");
    setFClsColor(item?.color ?? "#3b82f6");
    setFClsPrint(item?.printSubjectPicture ?? false);
    setFClsCustom(item?.customFields?.note ?? "");
    if (!item) {
  ...
  function openClassroomModal(item?: TClassroom) {
    setEditingItem(item ?? null);
    setFRmName(item?.name ?? "");
    setFRmShort(item?.short ?? "");
    setFRmColor(item?.color ?? "#14b8a6");
    setFRmCustom(item?.customFields?.note ?? "");
    setShowClassroomModal(true);
  }
  function openTeacherModal(item?: TTeacher) {
    setEditingItem(item ?? null);
    setFTLast(item?.lastName ?? "");
    setFTFirst(item?.firstName ?? "");
  ...
  function openLessonModal(item?: TLesson, teacherDefault?: TTeacher) {
    setEditingItem(item ?? null);
    setFLTeacher(item?.teacherId ?? teacherDefault?.id ?? "");
  ...
  function openContractPanel(teacher: TTeacher) {
    setContractTeacher(teacher);
  ...
  async function savePeriodTimes() {
    ...
    if (res.ok) {
      const updated = await res.json();
      setCurrent((prev) =>
        prev ? { ...prev, periodTimes: updated.periodTimes } : null,
      );
  ...
  function buildGrid() {
    ...
    const isBlocked = (time: string) =>
      offRules.length > 0 &&
      offRules.some((r) => r.from && r.to && time >= r.from && time < r.to);

    const khmerToArabic = (s: string) =>
      s.replace(/[០-៩]/g, (d) => String(d.charCodeAt(0) - 0x17e0));
    const gradeNum = (s: string) => {
      const d = khmerToArabic(s).match(/\d+/);
      return d ? parseInt(d[0], 10) : 9999;
    };
    const classes = [...current.classes].sort((a, b) => {
      const gA = gradeNum(a.short),
        gB = gradeNum(b.short);
      if (gA !== gB) return gA - gB;
      return khmerToArabic(a.name).localeCompare(
        khmerToArabic(b.name),
        undefined,
    ...
    function renderCell(
      ...
      const entry = entries.find(
        (e) => e.classId === cls.id && e.day === day && e.period === period,
      );
      ...
      if (blocked) {
        return (
          <td
            key={cellKey}
            className="border border-gray-200 dark:border-slate-700 p-0 text-center align-middle"
            style={{
              minWidth: 56,
              height: 46,
              background:
                "repeating-linear-gradient(135deg,#f3f4f6 0px,#f3f4f6 5px,#e5e7eb 5px,#e5e7eb 10px)",
            }}
          >
            {entry ? (
              ...
              <div
                ...
                style={{
                  backgroundColor: entry.subject.color ?? "#6366f1",
                  opacity: 0.75,
                }}
                draggable
                onDragStart={(e) => {
              ...
              >
                ...
                <div
                  className="opacity-90 text-[9px] rounded px-0.5 mt-0.5 inline-block"
      ...
      // ── Normal cell ──────────────────────────────────────────────────
      const isTarget =
        dropTarget?.classId === cls.id &&
        dropTarget?.day === day &&
        dropTarget?.period === period;
      return (
        <td
          key={cellKey}
          ...
          onDrop={async (e) => {
            ...
            if (ok && srcId) {
              const delRes = await apiFetch(`/api/timetable/entries/${srcId}`, {
                method: "DELETE",
              });
              if (delRes.ok)
                setCurrent((prev) =>
                  prev
                    ? {
                        ...prev,
                        entries: prev.entries.filter((e) => e.id !== srcId),
                      }
                    : prev,
                );
            }
          ...
          {entry ? (
            ...
            >
              ...
              <div
                className="opacity-90 text-[9px] rounded px-0.5 mt-0.5 inline-block"
                style={{
                  backgroundColor: entry.teacher.color ?? "#374151",
            ...
            <div
              className={`w-full h-full flex items-center justify-center text-gray-200 text-[9px] ${isTarget ? "text-brand-400" : ""}`}
            >
              {isTarget ? "↓" : ""}
            </div>
          )}
        ...
        >
          <thead>
            <tr className="bg-brand-700 text-white">
              <th
                rowSpan={2}
                className="border border-brand-800 px-2 py-1 min-w-[90px] text-left align-middle"
              >
                {t("timetable.class")}
              </th>
              {days.map((d) => (
                <th
                  key={d}
                  colSpan={dayColSpan}
                  className="border border-brand-800 px-2 py-1 text-center"
                >
                ...
                <Fragment key={d}>
                  {morningTimes.map((time, idx) =>
                    isBlocked(time) ? (
                      <th
                        key={`${d}-m${idx}`}
                        className="border border-orange-700 bg-orange-800/70 text-orange-200 px-0.5 py-1 text-[9px] font-normal text-center"
                        title={`Time-off: ${time}`}
                      >
                        ⊘<br />
                        {time}
                      ...
                      <th
                        key={`${d}-m${idx}`}
                        className="border border-brand-700 px-1 py-1 min-w-[56px] text-center font-normal"
                      >
                        {time}
                      </th>
                    ),
                  ...
                  {hasBrk && (
                    <th className="border border-brand-900 bg-brand-900/60 text-brand-400 px-1 py-1 text-[9px] font-normal">
                      ☕<br />
                      {t("timetable.break")}
                    </th>
                  )}
          ...
          <tbody>
            {classes.map((cls) => (
              <tr key={cls.id} className="even:bg-gray-50">
                <td className="border border-gray-300 dark:border-slate-600 px-2 py-1 align-middle whitespace-nowrap">
                  <div className="flex items-center gap-1.5">
                    ...
                    <div>
                      <div className="font-semibold text-xs text-gray-800 dark:text-slate-100">
                        {cls.short}
                      </div>
                ...
                {days.map((day) => (
                  <Fragment key={day}>
                    ...
                    {hasBrk && (
                      <td
                        className="border border-gray-200 dark:border-slate-700 bg-gray-100 dark:bg-slate-800"
                        style={{ width: 28 }}
                      />
                    )}
                    {afternoonTimes.map((time, idx) =>
  ...
  return (
    <AuthGuard allowedRoles={["ADMIN"]}>
      <div className="flex min-h-screen lg:h-screen bg-gray-100 dark:bg-slate-800 print:bg-white print:h-auto print:block pt-14 lg:pt-0 pb-[72px] lg:pb-0">
        ...
        <div className="flex-1 flex flex-col overflow-hidden print:hidden">
          ...
          <div className="bg-white dark:bg-slate-900 border-b-2 border-brand-100 dark:border-brand-900 select-none">
            ...
            {current && (
              <div className="bg-brand-700 text-white text-xs px-4 py-0.5 flex items-center gap-2">
                ...
                <span
                  className={`ml-2 px-1.5 py-0 rounded text-[10px] font-medium ${current.status === "PUBLISHED" ? "bg-green-400/30" : "bg-yellow-400/30"}`}
                >
                  {current.status}
                </span>
              </div>
            ...
            <div className="flex items-stretch gap-0 overflow-x-auto">
              {/* ── Group: File ── */}
              <div className="flex flex-col items-center border-r border-gray-200 dark:border-slate-700 px-1">
                <div className="flex items-end gap-1 py-1.5 flex-1">
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-gray-100 text-gray-700 dark:text-slate-200 transition-colors min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M3.75 9.75h16.5m-16.5 0A2.25 2.25 0 016 7.5h2.25L9.75 6h4.5l1.5 1.5H18a2.25 2.25 0 012.25 2.25v8.25A2.25 2.25 0 0118 20.25H6a2.25 2.25 0 01-2.25-2.25V9.75z"
                      />
                    </svg>
                  ...
                  <button
                    onClick={handleSave}
                    disabled={!current}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-gray-100 text-gray-700 dark:text-slate-200 transition-colors disabled:opacity-35 min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M16.5 3.75H6A2.25 2.25 0 003.75 6v12A2.25 2.25 0 006 20.25h12A2.25 2.25 0 0020.25 18V7.5L16.5 3.75z"
                      />
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M8.25 20.25v-6.75h7.5v6.75M15.75 3.75V8.25H9"
                      />
                    </svg>
                    <span className="text-[10px] leading-none">
                      {t("timetable.save")}
                  ...
                  >
                    ...
                    >
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M6.75 15.75H5.25a2.25 2.25 0 01-2.25-2.25v-4.5A2.25 2.25 0 015.25 6.75h13.5A2.25 2.25 0 0121 9v4.5a2.25 2.25 0 01-2.25 2.25h-1.5"
                      />
                      <path
                        strokeLinecap="round"
                        strokeLinejoin="round"
                        d="M6.75 6.75V3.75h10.5v3M6.75 15.75v4.5h10.5v-4.5"
                      />
                    </svg>
                    <span className="text-[10px] leading-none">
                      {t("timetable.print")}
                ...
                <span className="text-[9px] text-gray-400 pb-0.5 font-medium tracking-wide uppercase">
                  {t("timetable.file")}
                </span>
              </div>
              ...
              <div className="flex flex-col items-center border-r border-gray-200 dark:border-slate-700 px-1">
                <div className="flex items-end gap-1 py-1.5 flex-1">
                  {/* Subjects */}
                  <button
                    onClick={() => router.push("/admin/timetable/subjects")}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-blue-50 dark:hover:bg-blue-950/40 text-blue-700 dark:text-blue-300 transition-colors min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
                  ...
                  {/* Classes */}
                  <button
                    onClick={() => router.push("/admin/timetable/classes")}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-blue-50 dark:hover:bg-blue-950/40 text-blue-700 dark:text-blue-300 transition-colors min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                  ...
                  {/* Classrooms */}
                  <button
                    onClick={() => router.push("/admin/timetable/classrooms")}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-blue-50 dark:hover:bg-blue-950/40 text-blue-700 dark:text-blue-300 transition-colors min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                  ...
                  {/* Lessons */}
                  <button
                    onClick={() => router.push("/admin/timetable/lessons")}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-blue-50 dark:hover:bg-blue-950/40 text-blue-700 dark:text-blue-300 transition-colors min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
              ...
              <div className="flex flex-col items-center border-r border-gray-200 dark:border-slate-700 px-1">
                <div className="flex items-end gap-1 py-1.5 flex-1">
                  ...
                  <button
                    ...
                    disabled={!current}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-purple-50 dark:hover:bg-purple-950/40 text-purple-700 dark:text-purple-300 transition-colors disabled:opacity-35 min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
              ...
              <div className="flex flex-col items-center border-r border-gray-200 dark:border-slate-700 px-1">
                <div className="flex items-end gap-1 py-1.5 flex-1">
                  ...
                  <button
                    onClick={() =>
                      setZoom((z) =>
                        Math.max(0.5, parseFloat((z - 0.1).toFixed(1))),
                      )
                    }
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-gray-100 text-gray-700 dark:text-slate-200 transition-colors min-w-[40px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
                  ...
                  <button
                    onClick={() => setZoom(1)}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-gray-100 text-gray-600 dark:text-slate-300 transition-colors min-w-[40px]"
                  >
                  ...
                  <button
                    onClick={() =>
                      setZoom((z) =>
                        Math.min(2, parseFloat((z + 0.1).toFixed(1))),
                      )
                    }
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-gray-100 text-gray-700 dark:text-slate-200 transition-colors min-w-[40px]"
                  >
              ...
              <div className="flex flex-col items-center border-r border-gray-200 dark:border-slate-700 px-1">
                <div className="flex items-end gap-1 py-1.5 flex-1">
                  {/* Schedule */}
                  <button
                    onClick={() => router.push("/admin/timetable/schedule")}
                  ...
                  <button
                    ...
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-emerald-50 dark:hover:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 transition-colors disabled:opacity-35 min-w-[48px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
                  ...
                  <button
                    ...
                    disabled={!current}
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-emerald-50 dark:hover:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 transition-colors disabled:opacity-35 min-w-[52px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
              ...
              <div className="flex flex-col items-center px-1">
                <div className="flex items-end gap-1 py-1.5 flex-1">
                  <button
                    ...
                    disabled={!current || current.status === "PUBLISHED"}
                    title={
                      current?.status === "PUBLISHED"
                        ? "Unpublish before deleting"
                        : "Delete timetable"
                    }
                    className="flex flex-col items-center gap-0.5 px-2 py-1.5 rounded-md hover:bg-red-50 dark:hover:bg-red-950/40 text-red-500 dark:text-red-400 transition-colors disabled:opacity-35 disabled:cursor-not-allowed min-w-[44px]"
                  >
                    <svg
                      className="w-6 h-6"
                      viewBox="0 0 24 24"
          ...
          <div className="flex-1 overflow-hidden flex">
            ...
            {current && (
              <div
                className={`flex-shrink-0 bg-white border-r border-gray-200 flex flex-col transition-all duration-200 print:hidden ${showLessonPanel ? "w-48" : "w-8"}`}
              >
                <div className="flex items-center justify-between px-2 py-2 border-b border-gray-200 dark:border-slate-700">
                  {showLessonPanel && (
                    <span className="text-xs font-semibold text-gray-600 dark:text-slate-300 truncate">
                      {t("timetable.lessons")}
                    </span>
                  )}
                  <button
                    onClick={() => setShowLessonPanel((p) => !p)}
                ...
                {showLessonPanel && (
                  <>
                    {/* Class filter */}
                    <div className="px-2 py-1.5 border-b border-gray-100 dark:border-slate-800">
                      <select
                            ...
                            return (
                              ...
                              >
                                <div className="flex items-center justify-between">
                                  ...
                                  <span
                                    className={`text-[9px] font-bold px-1 rounded ml-1 flex-shrink-0 ${full ? "bg-green-400/80" : placed > 0 ? "bg-yellow-400/80 text-yellow-900" : "bg-black/25"}`}
                                  >
              ...
              {!loading && current && (
                <div>
                  <p className="text-[10px] text-gray-400 mb-2 print:hidden">
                    ← Drag a lesson card onto a cell to place it · Hover an
                    entry and click <strong>×</strong> to remove it · Drag an
                    entry to move it
                  </p>
                  {buildGrid()}
                </div>
          ...
          {/* Footer zoom */}
          <div className="bg-white dark:bg-slate-900 border-t border-gray-200 dark:border-slate-700 px-4 py-2 flex items-center gap-3 print:hidden">
            ...
            <button
              ...
              className="w-7 h-7 rounded border border-gray-300 dark:border-slate-600 text-gray-600 dark:text-slate-300 hover:bg-gray-100 flex items-center justify-center font-bold"
            >
              −
            </button>
            ...
            <button
              onClick={() => setZoom(1)}
              className="text-xs text-brand-600 dark:text-brand-400 hover:underline ml-1"
            >
              {t("timetable.zoomReset")}
            </button>
            {current && (
              <span className="ml-auto text-xs text-gray-400">
                {current.classes.length} classes · {current.entries.length}{" "}
                entries · {current.periodsPerDay} periods/day
              </span>
            )}
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-sm">
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">
                  {t("timetable.timeOffRules")}
                </label>
                <TimeOffEditor
                  rules={sTimeOffRules}
                  onChange={setSTimeOffRules}
                />
              </div>
              <p className="text-xs text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900 rounded px-3 py-2">
            ...
            <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 flex justify-end gap-2">
              ...
              <button
                ...
                disabled={savingSettings || sPeriods < 1 || sDays < 1}
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-sm">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
              <div>
                <h2 className="font-bold text-gray-800 dark:text-slate-100">
                  {t("timetable.periodTimes")}
                </h2>
                <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                  {t("timetable.setStartTime")}
                </p>
              </div>
              <button
            ...
            <div className="px-5 py-4 space-y-2 max-h-80 overflow-y-auto">
              {periodInputs.map((time, idx) => (
                <div key={idx} className="flex items-center gap-3">
                  <span className="text-xs font-semibold text-brand-600 dark:text-brand-400 w-16 flex-shrink-0">
                    {t("timetable.period")} {idx + 1}
                  </span>
                  <input
                    type="text"
                    placeholder="HH:MM"
            ...
            <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 rounded-b-xl">
        ...
        >
          <div className="space-y-1 max-h-80 overflow-y-auto">
            ...
            {timetableList.map((tt) => (
              <div
                key={tt.id}
                className="flex items-center gap-2 rounded border border-transparent hover:border-brand-200 hover:bg-brand-50 dark:hover:bg-brand-950/40 px-3 py-2 transition-colors"
              >
                <button
                ...
                >
                  ...
                  <div className="text-xs text-gray-500 dark:text-slate-400">
                    {tt.academicYear} · {tt.status}
                  </div>
                </button>
                {tt.status !== "PUBLISHED" && (
                  <button
                    onClick={async () => {
                      await handleDeleteTimetable(tt.id, tt.name, tt.status);
                    }}
                    className="shrink-0 px-2 py-1 text-xs text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/40 rounded border border-transparent hover:border-red-200"
                  >
                    {t("common.delete")}
                  </button>
                )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-3xl max-h-[92vh] flex flex-col">
            ...
            <div className="px-6 py-4 border-b border-gray-200 dark:border-slate-700 bg-brand-50 dark:bg-brand-950/40 rounded-t-xl flex items-center justify-between">
              <div>
                <h2 className="text-lg font-bold text-brand-800 dark:text-brand-300">
                  {t("timetable.newTimetable")}
                </h2>
                <p className="text-sm text-brand-600 dark:text-brand-400 mt-0.5">
                  {t("timetable.step")} {wizardStep + 1} {t("timetable.of")}{" "}
                  {WIZARD_STEPS.length} — {WIZARD_STEPS[wizardStep]}
                </p>
              </div>
              <div className="flex items-center gap-1.5">
            ...
            {wizardStep === 0 && (
              <div className="px-6 pt-3 pb-1">
                <div className="grid grid-cols-8 gap-1 text-[10px] font-semibold text-gray-400 uppercase tracking-wide border-b border-gray-100 dark:border-slate-800 pb-1">
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div className="col-span-2">
                      <label className="label-sm">
                        Name of School / Timetable
                      </label>
                      <input
                        className="input-field"
                        value={wSchoolName}
                        onChange={(e) => setWSchoolName(e.target.value)}
                        placeholder="e.g. Wattaman School 2025"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        className="input-field"
                        value={wAcademicYear}
                        onChange={(e) => setWAcademicYear(e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        className="input-field"
                        value={wPeriods}
                        onChange={(e) => setWPeriods(+e.target.value)}
                        min={1}
                        max={20}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={wMaxOn}
                        onChange={(e) =>
                          setWMaxOn(e.target.value ? +e.target.value : "")
                        }
                        min={1}
                  ...
                  <div className="border-t pt-3 grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        className="input-field"
                        value={wDistrib}
                        onChange={(e) => setWDistrib(e.target.value)}
                        placeholder="e.g. max 2 same subject/day"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        className="input-field"
                        value={wHomework}
                        onChange={(e) => setWHomework(e.target.value)}
                        placeholder="Notes..."
                      />
                    </div>
                    <div>
              ...
              {wizardStep === 1 && current && (
                <WizardListStep
                  title="Subjects"
                  addLabel="Enter Subject"
                  items={current.subjects}
                  ...
                  onEdit={(s: TSubject) => openSubjectModal(s)}
                  onRemove={(s: TSubject) => removeSubject(s.id, s.name)}
                />
              )}
              ...
              {wizardStep === 2 && current && (
                <WizardListStep
                  title="Classes"
                  addLabel="Enter Class"
                  items={current.classes}
                  ...
                  renderRow={(c: TClass) => [
                    ...
                    c.short,
                    colorBadge(c.color, "  "),
                    c.printSubjectPicture ? "✓" : "—",
                  ]}
                  onEdit={(c: TClass) => openClassModal(c)}
                  onRemove={(c: TClass) => removeClass(c.id, c.name)}
                />
              )}
              ...
              {wizardStep === 3 && current && (
                <WizardListStep
                  title="Classrooms"
                  addLabel="Enter Classroom"
                  items={current.classrooms}
                  onNew={() => openClassroomModal()}
                  columns={["Name", "Short", "Color"]}
                  renderRow={(r: TClassroom) => [
                    r.name,
                    r.short,
                    colorBadge(r.color, "  "),
                  ]}
                  onEdit={(r: TClassroom) => openClassroomModal(r)}
                  onRemove={(r: TClassroom) => removeClassroom(r.id, r.name)}
                />
              )}
              ...
              {wizardStep === 4 && current && (
                <WizardListStep
                  title="Teachers"
                  addLabel="Enter Teacher"
                  items={current.teachers}
                  ...
                  onRemove={(t: TTeacher) =>
                    removeTeacher(t.id, `${t.firstName} ${t.lastName}`.trim())
                  }
                  extraAction={(t: TTeacher) => (
                    <button
                      onClick={() => openContractPanel(t)}
                      className="text-emerald-600 dark:text-emerald-400 hover:underline mr-2 text-xs font-medium"
                    >
                      Contract
                    </button>
                  )}
            ...
            {/* Error message */}
            {wizardError && (
              <div className="mx-6 mb-1 px-3 py-2 bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 rounded-lg text-red-700 dark:text-red-300 text-sm">
                {wizardError}
              </div>
            )}

            {/* Footer */}
            <div className="px-6 py-3 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between bg-gray-50 dark:bg-slate-800 rounded-b-xl">
              <button
                onClick={() => setShowWizard(false)}
                className="tt-btn bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-200"
              >
                {t("common.cancel")}
              </button>
              <div className="flex gap-2">
                {wizardStep > 0 && (
                  <button
                    onClick={wizardPrev}
                    className="tt-btn bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200"
                  >
                    ← Previous
                  </button>
                )}
                {wizardStep < WIZARD_STEPS.length - 1 ? (
                  <button
                    ...
                    className="tt-btn bg-brand-600 text-white disabled:opacity-50"
                  >
                  ...
                  <button
                    onClick={wizardOk}
                    className="tt-btn bg-emerald-600 text-white"
                  >
                    OK ✓
                  </button>
                )}
      ...
      {showContractPanel && contractTeacher && current && (
        <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between bg-emerald-50 dark:bg-emerald-950/40 rounded-t-xl">
              <div>
                <h2 className="font-bold text-gray-800 dark:text-slate-100">
                  {t("timetable.lessonContracts")}
                </h2>
                <p className="text-sm text-gray-500 dark:text-slate-400">
                  {contractTeacher.lastName} {contractTeacher.firstName}
                </p>
              </div>
              <button
                onClick={() => openLessonModal(undefined, contractTeacher)}
                className="tt-btn bg-emerald-600 text-white text-xs"
              >
                {t("timetable.newLessonBtn")}
              </button>
            </div>
            <div className="flex-1 overflow-y-auto px-5 py-3">
              {current.lessons.filter((l) => l.teacherId === contractTeacher.id)
                .length === 0 ? (
                <p className="text-gray-400 text-sm text-center py-10">
                  {t("timetable.noContractsYet")}
                </p>
              ) : (
                <table className="w-full text-xs border-collapse">
                  <thead>
                    <tr className="bg-gray-50 dark:bg-slate-800">
                      {["Subject", "Class", "Lessons/Week", "Type", ""].map(
                        (h) => (
                          <th
                            key={h}
                            className="border-b border-gray-200 dark:border-slate-700 px-3 py-2 text-left font-semibold text-gray-600 dark:text-slate-300"
                          >
                            {h}
                          </th>
                        ),
                  ...
                  <tbody>
                    {current.lessons
                      .filter((l) => l.teacherId === contractTeacher.id)
                      .map((l) => (
                        <tr
                          key={l.id}
            ...
            <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 flex justify-end bg-gray-50 dark:bg-slate-800 rounded-b-xl">
              <button
      ...
      {showSubjectModal && (
        <ItemModal
          title={editingItem ? "Edit Subject" : "New Subject"}
          onOk={saveSubject}
        ...
        >
          ...
          <Field label="Short Name">
            <input
              ...
              style={{ fontFamily: '"Khmer", Inter, Arial, sans-serif' }}
              value={fSubShort}
              onChange={(e) => setFSubShort(e.target.value)}
              maxLength={8}
          ...
          <Field label="Custom Fields">
            <input
              className="input-field"
              value={fSubCustom}
              onChange={(e) => setFSubCustom(e.target.value)}
          ...
          <Field label="Classroom Count">
            <div className="flex items-center gap-2">
              <input
                ...
                style={{ width: 80 }}
                value={fSubRooms}
                onChange={(e) => setFSubRooms(+e.target.value)}
                min={1}
              />
          ...
          <Field label="Color / Picture">
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ backgroundColor: fSubColor }}
              />
              <button
                type="button"
                onClick={async () => {
                  const c = await openColorPicker(fSubColor);
                  setFSubColor(c);
                }}
                className="tt-btn bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 text-xs"
              >
                Change
              </button>
              <span className="text-xs text-gray-500 dark:text-slate-400">
          ...
          <Field label="Set for lessons of this subject">
            <p className="text-xs text-gray-400">
              Classroom count sets how many rooms this subject requires.
            </p>
          </Field>
        </ItemModal>
      )}
        ...
        >
                  ...
                  {wizardSystemClasses
                    ...
                    .map((sc) => (
                      <button
                        ...
                        onClick={() => {
                          setFClsName(sc.name);
                          setFClsShort(autoShort(sc.name));
                        }}
                      ...
                      >
                        {sc.name}
                        {sc.studyYear
                          ? ` · ${sc.studyYear.label ?? sc.studyYear.year}`
                          : ""}
                      </button>
                    ))}
                  {!wizardSystemClassesLoading &&
                    wizardSystemClasses.filter(
                      (sc) =>
                        !(current?.classes ?? []).some(
                          (tc) =>
                            tc.name.toLowerCase() === sc.name.toLowerCase(),
                      ...
                      <p className="text-xs text-gray-400">
                        All existing classes are already added.
                      </p>
                    )}
                </div>
              )}
          ...
          <Field label="Class Name">
            <input
              ...
              style={{ fontFamily: '"Khmer", Inter, Arial, sans-serif' }}
              value={fClsName}
              onChange={(e) => setFClsName(e.target.value)}
          ...
          <Field label="Short Name">
            <input
              ...
              style={{ fontFamily: '"Khmer", Inter, Arial, sans-serif' }}
              value={fClsShort}
              onChange={(e) => setFClsShort(e.target.value)}
              maxLength={8}
          ...
          <Field label="Custom Fields">
            <input
              className="input-field"
              value={fClsCustom}
              onChange={(e) => setFClsCustom(e.target.value)}
              placeholder="Optional notes"
            />
          </Field>
          <Field label="Color / Picture">
            <div className="flex items-center gap-3">
              <div
                className="w-8 h-8 rounded border border-gray-300 dark:border-slate-600"
                style={{ backgroundColor: fClsColor }}
              />
              <button
                type="button"
                onClick={async () => {
                  const c = await openColorPicker(fClsColor);
                  setFClsColor(c);
                }}
                className="tt-btn bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 text-xs"
              >
          ...
          <label className="flex items-center gap-2 text-sm text-gray-700 dark:text-slate-200 cursor-pointer mt-1">
            <input
              type="checkbox"
              checked={fClsPrint}
              onChange={(e) => setFClsPrint(e.target.checked)}
              className="rounded"
            />
      ...
      {showClassroomModal && (
        <ItemModal
          title={editingItem ? "Edit Classroom" : "New Classroom"}
          onOk={saveClassroom}
        ...
        >
          ...
          <Field label="Short Name">
            <input
              className="input-field"
              value={fRmShort}
              onChange={(e) => setFRmShort(e.target.value)}
              maxLength={8}
          ...
          <Field label="Custom Fields">
            <input
              className="input-field"
              value={fRmCustom}
              onChange={(e) => setFRmCustom(e.target.value)}
              placeholder="Optional notes"
            />
          </Field>
          <Field label="Color / Picture">
            <div className="flex items-center gap-3">
              <div
                className="w-8 h-8 rounded border border-gray-300 dark:border-slate-600"
                style={{ backgroundColor: fRmColor }}
              />
              <button
                type="button"
                onClick={async () => {
                  const c = await openColorPicker(fRmColor);
                  setFRmColor(c);
                }}
                className="tt-btn bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 text-xs"
              >
      ...
      {showTeacherModal && (
        <ItemModal
          title={editingItem ? "Edit Teacher" : "New Teacher"}
          onOk={saveTeacher}
        ...
        >
                ...
                <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
                  ...
                  {!systemTeachersLoading &&
                    systemTeachers.filter(
                      ...
                      <p className="text-xs text-gray-400">
                        All existing teachers are already added, or no teachers
                        found in the system.
                      </p>
                    )}
                </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Last Name">
              <input
                ...
                style={{ fontFamily: '"Khmer", Inter, Arial, sans-serif' }}
                value={fTLast}
                onChange={(e) => setFTLast(e.target.value)}
              />
            </Field>
            <Field label="First Name">
              <input
                ...
                style={{ fontFamily: '"Khmer", Inter, Arial, sans-serif' }}
                value={fTFirst}
                onChange={(e) => setFTFirst(e.target.value)}
              />
            </Field>
          </div>
          <Field label="Short Name">
            <input
              ...
              style={{ fontFamily: '"Khmer", Inter, Arial, sans-serif' }}
              value={fTShort}
              onChange={(e) => setFTShort(e.target.value)}
              maxLength={8}
            />
          </Field>
          <Field label="Sex">
            <div className="flex gap-6 mt-1">
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
                  checked={fTSex === "MALE"}
                  onChange={() => setFTSex(fTSex === "MALE" ? "" : "MALE")}
                />
                Male
              </label>
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input
                  type="checkbox"
                  checked={fTSex === "FEMALE"}
                  onChange={() => setFTSex(fTSex === "FEMALE" ? "" : "FEMALE")}
                />
                Female
              </label>
            </div>
          ...
          <Field label="Class Teacher for (Class)">
            <div className="flex items-center gap-2">
              <select
                className="input-field"
                value={fTClassTeacher}
                onChange={(e) => setFTClassTeacher(e.target.value)}
              >
              ...
              <button
                type="button"
                onClick={() => setFTClassTeacher("")}
                className="tt-btn bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 text-xs whitespace-nowrap"
              >
                Clear
              </button>
            </div>
          ...
          <Field label="Color">
            <div className="flex items-center gap-3">
              <div
                className="w-8 h-8 rounded border border-gray-300 dark:border-slate-600"
                style={{ backgroundColor: fTColor }}
              />
              <button
                type="button"
                onClick={async () => {
                  const c = await openColorPicker(fTColor);
                  setFTColor(c);
                }}
                className="tt-btn bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 text-xs"
              >
      ...
      {showLessonModal && (
        <ItemModal
          title={editingItem ? "Edit Lesson" : "New Lesson"}
          onOk={saveLesson}
        ...
        >
          <Field label="Teacher">
            <select
              className="input-field"
              value={fLTeacher}
              onChange={(e) => setFLTeacher(e.target.value)}
            >
              <option value="">Select teacher</option>
              {current?.teachers.map((t: TTeacher) => (
          ...
          <Field label="Subject">
            ...
            >
              ...
              {current?.subjects.map((s: TSubject) => (
          ...
          <Field label="Class">
            <select
              className="input-field"
              value={fLClass}
              onChange={(e) => setFLClass(e.target.value)}
            >
          ...
          <Field label="Lessons / Week">
            <select
              className="input-field"
              value={fLPerWeek}
              onChange={(e) => setFLPerWeek(+e.target.value)}
            >
      ...
      {showWorkloadModal &&
        ...
        (() => {
          ...
          const rows = current.teachers.map((teacher) => {
            const teacherEntries = current.entries.filter(
            ...
            const subjects = Object.values(subjectMap).sort(
              (a, b) => b.count - a.count,
            );
            return {
              teacher,
          ...
          return (
            <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
              <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col">
                ...
                <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between bg-emerald-50 dark:bg-emerald-950/40 rounded-t-xl">
                  <div>
                    <h2 className="font-bold text-gray-800 dark:text-slate-100 text-base">
                      {t("timetable.teacherWorkload")}
                    </h2>
                    <p className="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
                      {current.name} · {current.academicYear} ·{" "}
                      {current.teachers.length} teachers
                    </p>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-2">
                      <label className="text-xs text-gray-600 dark:text-slate-300 font-medium">
                        {t("timetable.weeksPerMonth")}
                      </label>
                      <select
                        value={weeksPerMonth}
                        onChange={(e) => setWeeksPerMonth(+e.target.value)}
                        className="border border-gray-300 dark:border-slate-600 rounded-lg px-2 py-1 text-xs focus:outline-none focus:ring-2 focus:ring-emerald-500"
                      >
                        {[3, 4, 5].map((n) => (
                          <option key={n} value={n}>
                            {n}
                ...
                {/* Summary cards */}
                <div className="px-5 py-3 grid grid-cols-3 gap-3 border-b border-gray-100 dark:border-slate-800">
                  <div className="bg-emerald-50 dark:bg-emerald-950/40 rounded-lg px-3 py-2 text-center">
                    <div className="text-xl font-bold text-emerald-700 dark:text-emerald-300">
                      {totalWeek}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-slate-400">
                      {t("timetable.totalPeriodsWeek")}
                    </div>
                  </div>
                  <div className="bg-blue-50 dark:bg-blue-950/40 rounded-lg px-3 py-2 text-center">
                    <div className="text-xl font-bold text-blue-700 dark:text-blue-300">
                      {totalMonth}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-slate-400">
                      {t("timetable.totalPeriodsMonth")} ({weeksPerMonth}w)
                    </div>
                  </div>
                  <div className="bg-brand-50 dark:bg-brand-950/40 rounded-lg px-3 py-2 text-center">
                    <div className="text-xl font-bold text-brand-700 dark:text-brand-300">
                      {current.teachers.length > 0
                        ? (totalWeek / current.teachers.length).toFixed(1)
                        : 0}
                    </div>
                    <div className="text-xs text-gray-500 dark:text-slate-400">
                      {t("timetable.avgPeriodsTeacher")}
                    </div>
                  </div>
                </div>

                {/* Sort controls */}
                <div className="px-5 py-2 flex items-center gap-2 border-b border-gray-100 dark:border-slate-800">
                  <span className="text-xs text-gray-500 dark:text-slate-400">
                    {t("timetable.sortBy")}
                  </span>
                  {(["name", "week", "month"] as const).map((s) => (
                    ...
                    >
                      {s === "name"
                        ? t("timetable.sortName")
                        : s === "week"
                          ? t("timetable.sortWeek")
                          : t("timetable.sortMonth")}
                    </button>
                  ))}
                </div>
                  ...
                  {sorted.length === 0 ? (
                    ...
                    <table className="w-full text-xs border-collapse">
                      <thead className="sticky top-0 bg-gray-50 dark:bg-slate-800">
                        <tr>
                          <th className="border-b border-gray-200 dark:border-slate-700 px-4 py-2 text-left font-semibold text-gray-600 dark:text-slate-300">
                            {t("timetable.teacher")}
                          </th>
                          <th className="border-b border-gray-200 dark:border-slate-700 px-3 py-2 text-center font-semibold text-gray-600 dark:text-slate-300">
                            {t("timetable.contractedPerWeek")}
                          </th>
                          <th className="border-b border-gray-200 dark:border-slate-700 px-3 py-2 text-center font-semibold text-gray-600 dark:text-slate-300">
                            {t("timetable.scheduledPerWeek")}
                          </th>
                          <th className="border-b border-gray-200 dark:border-slate-700 px-3 py-2 text-center font-semibold text-gray-600 dark:text-slate-300">
                            {t("timetable.periodsPerMonth")}
                          </th>
                          <th className="border-b border-gray-200 dark:border-slate-700 px-4 py-2 text-left font-semibold text-gray-600 dark:text-slate-300">
                            {t("timetable.subjectsPlaced")}
                          </th>
                          <th className="border-b border-gray-200 dark:border-slate-700 px-3 py-2 text-left font-semibold text-gray-600 dark:text-slate-300 w-32">
                            {t("timetable.loadBar")}
                          </th>
                        </tr>
                      </thead>
                      <tbody>
                        {sorted.map((row, idx) => {
                          const diff =
                            row.periodsPerWeek - row.contractedPerWeek;
                          const barPct = Math.round(
                            (row.periodsPerWeek / maxWeek) * 100,
                          );
                          ...
                          return (
                            ...
                            >
                              ...
                              <td className="px-3 py-2.5 text-center">
                                ...
                                {diff !== 0 && (
                                  <span
                                    className={`ml-1 text-[10px] font-medium ${overload ? "text-orange-500" : "text-red-400"}`}
                                  >
                                    ({overload ? "+" : ""}
                                    {diff})
                                  </span>
                                ...
                                <div className="flex flex-wrap gap-1">
                                  {row.subjects.length === 0 ? (
                                    <span className="text-gray-300">
                                      {t("timetable.noEntries")}
                                    </span>
                                  ) : (
                                    row.subjects.map((s) => (
                                      <span
                                        key={s.name}
                                        className="inline-flex items-center gap-0.5 rounded px-1.5 py-0.5 text-white text-[10px] font-medium"
                                        style={{
                ...
                {/* Footer legend */}
                <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800 rounded-b-xl flex items-center justify-between gap-4">
                  <div className="flex items-center gap-4 text-[10px] text-gray-500 dark:text-slate-400">
                    <span className="flex items-center gap-1">
                      ...
                      {t("timetable.matchedContracted")}
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="w-3 h-2 rounded bg-orange-500 inline-block" />{" "}
                      {t("timetable.overContracted")}
                    </span>
                    <span className="flex items-center gap-1">
                      <span className="w-3 h-2 rounded bg-red-500 inline-block" />{" "}
                      {t("timetable.underScheduled")}
                    </span>
                  </div>
                  <button
                    onClick={() => setShowWorkloadModal(false)}
                    className="px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-lg text-sm font-medium hover:bg-gray-200"
                  >
                    {t("common.close")}
                  </button>
                </div>
      ...
      {showPrintModal && current && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
            <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between flex-shrink-0">
              <h2 className="font-bold text-gray-800 dark:text-slate-100">
                {t("timetable.printTitle")}
              </h2>
              <button
            ...
            <div className="px-5 py-5 space-y-5 overflow-y-auto">
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-2">
                  {t("timetable.whatToPrint")}
                </label>
                <div className="space-y-2">
                  <label
                    className="flex items-center gap-3 p-3 rounded-lg border-2 cursor-pointer transition-colors"
                    style={{
                      borderColor: printMode === "all" ? "#6366f1" : "#e5e7eb",
                      backgroundColor: printMode === "all" ? "#eef2ff" : "",
                    }}
                  >
                    <input
                      type="radio"
                    ...
                    />
                    <div>
                      <div className="font-medium text-sm text-gray-800 dark:text-slate-100">
                        {t("timetable.wholeTimetable")}
                      </div>
                      <div className="text-xs text-gray-500 dark:text-slate-400">
                        {t("timetable.onePagePerClass")} — all{" "}
                        {current.classes.length} classes
                      </div>
                    </div>
                  </label>
                  <label
                    ...
                    style={{
                      borderColor:
                        printMode === "class" ? "#6366f1" : "#e5e7eb",
                      backgroundColor: printMode === "class" ? "#eef2ff" : "",
                    }}
                  >
                    <input
                      type="radio"
                      ...
                      onChange={() => {
                        setPrintMode("class");
                        if (!printClassId && current.classes.length > 0)
                          setPrintClassId(current.classes[0].id);
                      }}
                      className="accent-brand-600"
                    />
                    <div className="flex-1">
                      <div className="font-medium text-sm text-gray-800 dark:text-slate-100">
                        {t("timetable.singleClass")}
                      </div>
                      <div className="text-xs text-gray-500 dark:text-slate-400">
                        {t("timetable.oneClassOnly")}
                      </div>
                    </div>
                  </label>
                </div>
              ...
              <div>
                  ...
                  {printHeaderLines.map((line, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className="text-red-400 hover:text-red-600 dark:hover:text-red-400 px-2 text-lg font-bold flex-shrink-0"
                      >
                        ×
                      </button>
                    </div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && printNewHeaderLine.trim()) {
                        setPrintHeaderLines((prev) => [
                          ...prev,
                          printNewHeaderLine.trim(),
                        ]);
                        setPrintNewHeaderLine("");
                      }
                    }}
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter" && printNewSigner.trim()) {
                        setPrintSigners((prev) => [
                          ...prev,
                          printNewSigner.trim(),
                        ]);
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-600 dark:text-slate-300 mb-1">
                  Footer{" "}
                  <span className="text-gray-400 font-normal">
                    (left and right info above signature lines)
                  </span>
                </label>
                <div className="flex gap-2">
                  <input
            ...
            <div className="px-5 py-3 border-t border-gray-200 dark:border-slate-700 flex justify-end gap-2 flex-shrink-0">
              <button
                onClick={() => setShowPrintModal(false)}
                className="px-4 py-2 bg-gray-100 dark:bg-slate-800 text-gray-700 dark:text-slate-200 rounded-lg text-sm font-medium"
              >
        ...
        <div className="fixed inset-0 bg-black/40 z-[70] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-72 p-5">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => resolveColorPicker(colorPickerValue)}
                className="tt-btn bg-brand-600 text-white"
              >
                OK
              </button>
            </div>
...
function PrintLayout({
  ...
  const classesToPrint =
    mode === "class" ? allClasses.filter((c) => c.id === classId) : allClasses;

  const morningPeriods = times
    .map((t, i) => ({ time: t, period: i + 1 }))
    .filter((p) => p.time < "12:00");
  const afternoonPeriods = times
    .map((t, i) => ({ time: t, period: i + 1 }))
    .filter((p) => p.time >= "12:00");
  ...
  function getEntry(classId: string, day: number, period: number) {
    return (
      timetable.entries.find(
        (e) => e.classId === classId && e.day === day && e.period === period,
      ) ?? null
    );
  }
        ...
        >
          ...
          >
              ...
              {orgName && (
                <div
                  style={{
                    ...
                    letterSpacing: 1,
                    marginTop: headerLines.length > 0 ? 4 : 0,
                  }}
                >
                  {orgName}
                </div>
            ...
            <div style={{ textAlign: "center", marginTop: 10 }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: "#374151" }}>
                {t("timetable.printTitle") || "Class Timetable"}
              </div>
              <div
                style={{
                  marginTop: 4,
              ...
              >
                <span>
                  <strong>{t("timetable.academicYearLabel")}:</strong>{" "}
                  {timetable.academicYear}
                </span>
                <span>
                  <strong>{t("timetable.classLabel")}:</strong> {cls.name}
                </span>
              </div>
            </div>
          </div>
          ...
          <table style={{ borderCollapse: "collapse", width: "100%" }}>
            <thead>
              <tr>
                <th
                  style={{ ...headerStyle, minWidth: 110, textAlign: "left" }}
                >
                  Time
                </th>
                {days.map((d) => (
          ...
          >
            ...
            <div>
              {rightFooter || `${orgName || timetable.name} — ${cls.name}`}
            </div>
          </div>
          ...
          {signers.length > 0 && (
            <div
              style={{
                ...
                display: "flex",
                justifyContent:
                  signers.length <= 3 ? "space-between" : "space-around",
                flexWrap: "wrap" as const,
                gap: 32,
                padding: "0 16px",
              }}
  ...
  return (
    <div className="space-y-3">
      {disabled && disabledMessage && (
        <div className="rounded-lg border border-amber-200 dark:border-amber-900 bg-amber-50 dark:bg-amber-950/40 px-3 py-2 text-xs text-amber-700 dark:text-amber-300">
          {disabledMessage}
        </div>
      )}
      <div className="flex items-center justify-between">
        <h3 className="font-semibold text-gray-700 dark:text-slate-200">
          {addLabel}{" "}
        ...
        <button
          onClick={onNew}
          disabled={disabled}
          className="tt-btn bg-brand-600 text-white text-xs disabled:opacity-40 disabled:pointer-events-none"
        >
          + New
        </button>
      </div>
      {items.length === 0 ? (
        <div className="border-2 border-dashed border-gray-200 dark:border-slate-700 rounded-xl py-10 text-center text-gray-400 text-sm">
          ...
          <button
            onClick={onNew}
            disabled={disabled}
            className="mt-2 text-brand-600 dark:text-brand-400 hover:underline text-sm font-medium disabled:opacity-40 disabled:pointer-events-none"
          >
            + Add first
          </button>
        </div>
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
                {columns.map((c) => (
                  <th
                    key={c}
                    className="px-3 py-2 text-left font-semibold text-gray-600 dark:text-slate-300"
                  >
                    {c}
                  </th>
                ))}
                <th className="px-3 py-2 text-right font-semibold text-gray-600 dark:text-slate-300">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                <tr
                  key={item.id}
                  className="border-b border-gray-100 dark:border-slate-800 last:border-0 hover:bg-brand-50/40"
                >
                  ...
                  <td className="px-3 py-2 text-right whitespace-nowrap">
                    ...
                    <button
                      onClick={() => onEdit(item)}
                      disabled={disabled}
                      className="text-blue-600 dark:text-blue-400 hover:underline mr-2 disabled:opacity-40 disabled:pointer-events-none"
                    >
                      Edit
                    </button>
                    <button
                      ...
                      disabled={disabled}
                      className="text-red-500 dark:text-red-400 hover:underline disabled:opacity-40 disabled:pointer-events-none"
                    >
                      Remove
                    </button>
                  </td>
...
function ItemModal({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 z-[60] flex items-center justify-center p-4">
      <div
        className={`bg-white rounded-xl shadow-2xl w-full ${width} max-h-[85vh] flex flex-col`}
      >
        <div className="px-5 py-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
          <h2 className="font-bold text-gray-800 dark:text-slate-100">
            {title}
        ...
        <div className="px-5 py-3 border-t border-gray-100 dark:border-slate-800 flex justify-end gap-2 bg-gray-50 dark:bg-slate-800 rounded-b-xl">
          ...
          <button onClick={onOk} className="tt-btn bg-brand-600 text-white">
            OK
          </button>
        </div>
      </div>
