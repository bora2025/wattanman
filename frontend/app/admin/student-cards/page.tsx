                    ...
                    {classes.map((cls) => (
                      ...
                      >
                        ...
                        <div className="px-5 py-3 flex items-center justify-between bg-white dark:bg-slate-900">
                          ...
                          <span className="text-brand-400 group-hover:text-brand-600 text-sm transition-colors">View →</span>
                        </div>
                      </button>
                    ))}
                  </div>
                  ...
                  <div className="space-y-4">
                    <div className="card p-4">
                      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <button onClick={() => setSelectedStudentClass(null)} className="flex items-center gap-2 text-brand-600 dark:text-brand-400 hover:text-brand-800 dark:hover:text-brand-300 font-medium text-sm transition-colors shrink-0">← Back to Classes</button>
                        ...
                        <div className="sm:ml-auto flex items-center gap-2 flex-wrap">
                          <div className="relative flex-1 min-w-0 sm:min-w-[180px]">
                            ...
                            <input type="text" placeholder="Search students..." value={studentSearch} onChange={(e) => { setStudentSearch(e.target.value); setStudentPage(1); }} className="w-full pl-9 pr-3 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 focus:border-brand-300 focus:ring-2 focus:ring-brand-100 outline-none transition-all" />
                          </div>
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {Array.from({ length: totalStudentPages }, (_, i) => i + 1)
                              ...
                              .map((p, i) => typeof p === 'string' ? <span key={`dot-${i}`} className="px-1 text-xs text-slate-400 dark:text-slate-500">…</span> : <button key={p} onClick={() => setStudentPage(p)} className={`px-2.5 py-1 text-xs rounded-lg border transition-colors ${p === safePage ? 'bg-brand-600 text-white border-brand-600' : 'border-slate-200 hover:bg-slate-50'}`}>{p}</button>)}
