...
function ClassDetailContent() {
  ...
  const examsEnabled =
    enabledModules === null || enabledModules.includes("EXAMS");
  const visibleTabs = TABS.filter((t) => t !== "exams" || examsEnabled);
  useEffect(() => {
    if (enabledModules !== null && tab === "exams" && !examsEnabled)
      setTab("assignments");
    ...
    <div className="page-shell">
      ...
      <div className="page-content">
        ...
        <div className="page-header">
          <nav
            className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 mb-2"
          ...
          {loadingClass ? (
            ...
            <div className="flex items-start justify-between gap-3 flex-wrap">
              ...
              {(enabledModules === null ||
                ...
                <Link
                  href={`/admin/attendance?classId=${classId}`}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-slate-600 dark:text-slate-300">
          ...
          assignment{rows.length === 1 ? "" : "s"} in this class
        </p>
        <button
          onClick={() => {
            setEditing(null);
      ...
      {creating && (
        <form
          ...
          className="card p-4 space-y-3 bg-brand-50/30 border-brand-200 dark:border-brand-900"
        >
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, totalMarks: Number(e.target.value) }))
                }
                className="w-full border rounded-lg px-3 py-2 text-sm"
              />
            </div>
      ...
      {editing && (
        <form
          ...
          className="card p-4 space-y-3 bg-emerald-50/30 border-emerald-200 dark:border-emerald-900"
        >
          <h3 className="text-sm font-semibold text-slate-800 dark:text-slate-100">
            Edit assignment
          </h3>
      ...
      {error && (
        <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-900 text-red-700 dark:text-red-300 rounded-lg px-3 py-2 text-sm">
          {error}
        </div>
      )}
              ...
              {rows.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 text-xs text-slate-500 dark:text-slate-400">
                    {r.dueDate ? formatCambodiaTime(r.dueDate) : "—"}
                  </td>
                  ...
                  <td className="px-4 py-2 text-right whitespace-nowrap">
                    <button
                      onClick={() => openEdit(r)}
                      className="text-xs text-emerald-700 dark:text-emerald-300 hover:underline mr-3"
                    >
                      Edit
                    </button>
                    {r.type === "QUIZ" && (
                      <Link
                        href={`/teacher/assignments/${r.id}/quiz`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="text-xs text-violet-600 dark:text-violet-400 hover:underline mr-3"
                      >
                        Quiz Questions ↗
                      </Link>
                    )}
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-slate-600 dark:text-slate-300">
          ...
          exam{rows.length === 1 ? "" : "s"} in this class
        </p>
        <Link
          href={`/teacher/exams/new?classId=${classId}&returnTo=${returnTo}`}
          className="btn-primary btn-sm"
        >
          + New Exam
        </Link>
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 text-right whitespace-nowrap">
                    <Link
                      href={`/teacher/exams/${r.id}/edit?returnTo=${returnTo}`}
                      className="text-xs text-emerald-700 dark:text-emerald-300 hover:underline mr-3"
                    >
                      Edit
                    </Link>
                    <Link
                      href={`/teacher/exams/${r.id}/attempts`}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="text-xs text-brand-600 dark:text-brand-400 hover:underline mr-3"
                    >
...
function CoursesPanel({ classId }: { classId: string }) {
  ...
  const handleDelete = async (row: CourseRow) => {
    if (!confirm(`Delete course "${row.title}"? This cannot be undone.`))
      return;
    try {
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-sm text-slate-600 dark:text-slate-300">
          ...
          course{rows.length === 1 ? "" : "s"} in this class
        </p>
        <button
          onClick={() => {
            setEditing(null);
            ...
            <tbody>
              {rows.map((r) => (
                ...
                >
                  ...
                  <td className="px-4 py-2 text-xs text-slate-500 dark:text-slate-400">
                    {formatCambodiaTime(r.updatedAt)}
                  </td>
                  <td className="px-4 py-2">
                    <select
                      value={r.status}
