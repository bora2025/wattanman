    ...
    <div className="flex flex-col bg-[#1a1a1a] h-full">
        ...
        <div className="flex-1 min-w-0 flex flex-col overflow-hidden">
          ...
          >
            <div style={{ transform: `scale(${zoom / 100})`, transformOrigin: 'top center', transition: 'transform 0.15s', filter: 'drop-shadow(0 8px 32px rgba(0,0,0,0.6))', position: 'relative' }}>
              ...
              {isPreviewMode ? (
                <div
                  className="relative flex items-center justify-center bg-white dark:bg-slate-900 rounded-xl overflow-hidden"
      ...
      {showClearCache && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowClearCache(false)}>
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center gap-3 mb-3"><span className="text-3xl">🧹</span><h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">Clear Cache</h3></div>
            <p className="text-sm text-slate-600 dark:text-slate-300 mb-2">This will permanently delete:</p>
            <ul className="text-sm text-slate-600 dark:text-slate-300 mb-5 list-disc list-inside space-y-1"><li>All saved card designs</li><li>All saved custom templates</li></ul>
            <p className="text-xs text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/40 rounded-lg px-3 py-2 mb-5">⚠️ This cannot be undone.</p>
            <div className="flex justify-end gap-2">
              <button onClick={() => setShowClearCache(false)} className="px-4 py-2 text-sm rounded-lg border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">Cancel</button>
      ...
      {showTemplatePicker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={() => setShowTemplatePicker(false)}>
          <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl p-6 w-full max-w-2xl max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">🎨 Choose Template</h3>
              <button onClick={() => setShowTemplatePicker(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 text-2xl leading-none">&times;</button>
            </div>

            {isCertMode ? (
              <>
                {/* Certificate built-in */}
                <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">📜 Certificate Templates</h4>
                ...
                <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Your Saved Certificate Templates</h4>
                {!savedTemplatesLoaded ? (
                  <div className="grid grid-cols-2 gap-3">
                    {[1, 2].map((i) => (
                      <div key={i} className="rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 animate-pulse overflow-hidden">
                        <div className="h-28 bg-slate-100 dark:bg-slate-800" />
                        <div className="p-3 space-y-2"><div className="h-3 bg-slate-100 dark:bg-slate-800 rounded w-3/4" /><div className="h-2 bg-slate-100 dark:bg-slate-800 rounded w-1/2" /></div>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="text-center py-8 bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                    ...
                    <p className="text-sm text-slate-400 dark:text-slate-500">No saved certificate templates yet. Click &ldquo;As Template&rdquo; to save one.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-3">
                    {savedTemplates.filter((t) => t.cardType === 'certificate-student' || t.cardType === 'certificate-staff').map((tpl) => {
                      ...
                      <div key={tpl.id} className={`rounded-xl border-2 bg-white transition-all relative group overflow-hidden ${isActive ? 'border-emerald-500 ring-2 ring-emerald-300 shadow-md' : 'border-slate-200 hover:border-slate-400'}`}>
                        ...
                        <button onClick={() => handleLoadTemplate(tpl)} className="w-full text-left">
                          <div className="bg-slate-50 dark:bg-slate-800 flex items-center justify-center p-3 border-b border-slate-100 dark:border-slate-800 h-28">
                          ...
                          <div className="p-3">
                            <div className="flex items-center gap-2 mb-1"><span>{tpl.cardType === 'certificate-student' ? '📜' : '🏅'}</span><span className="font-semibold text-slate-800 dark:text-slate-100 truncate text-sm">{tpl.name}</span></div>
                            <span className="text-[10px] text-slate-400 dark:text-slate-500">{new Date(tpl.createdAt).toLocaleDateString()}</span>
                          </div>
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); handleDeleteTemplate(tpl.id); }} className="absolute top-2 right-2 w-6 h-6 rounded-full bg-red-50 dark:bg-red-950/40 text-red-400 hover:bg-red-100 hover:text-red-600 dark:hover:text-red-400 text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
                      </div>
                      );
                    })}
                  </div>
              ...
              <>
                ...
                <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Built-in</h4>
                ...
                {lockedType !== 'staff' && (
                  <>
                    <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">🎓 Student Styles</h4>
                ...
                {lockedType !== 'student' && (
                  <>
                    <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">👨‍🏫 Staff Styles</h4>
                    <div className="grid grid-cols-5 gap-2 mb-5">
                      {([
                ...
                <h4 className="text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Your Saved Templates</h4>
                {!savedTemplatesLoaded ? (
                  <div className="grid grid-cols-2 gap-3">
                    {[1, 2].map((i) => (
                      <div key={i} className="rounded-xl border-2 border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 animate-pulse overflow-hidden">
                  ...
                  <div className="text-center py-8 bg-slate-50 dark:bg-slate-800 rounded-xl border border-dashed border-slate-200 dark:border-slate-700">
                    <span className="text-3xl block mb-2">📭</span>
                    <p className="text-sm text-slate-400 dark:text-slate-500">No saved templates yet. Click &ldquo;As Template&rdquo; to save one.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-2 gap-3">
                    {savedTemplates.filter((t) => lockedType ? t.cardType === lockedType : (t.cardType === 'student' || t.cardType === 'staff')).map((tpl) => {
                      ...
                      <div key={tpl.id} className={`rounded-xl border-2 bg-white transition-all relative group overflow-hidden ${isActive ? 'border-emerald-500 ring-2 ring-emerald-300 shadow-md' : 'border-slate-200 hover:border-slate-400'}`}>
                        ...
                        <button onClick={() => handleLoadTemplate(tpl)} className="w-full text-left">
                          ...
                          <div className="p-3">
                            <div className="flex items-center gap-2 mb-1"><span>{tpl.cardType === 'student' ? '🎓' : '👨‍🏫'}</span><span className="font-semibold text-slate-800 dark:text-slate-100 truncate text-sm">{tpl.name}</span></div>
                            <span className="text-[10px] text-slate-400 dark:text-slate-500">{new Date(tpl.createdAt).toLocaleDateString()}</span>
                          </div>
                        </button>
                        <button onClick={(e) => { e.stopPropagation(); handleDeleteTemplate(tpl.id); }} className="absolute top-2 right-2 w-6 h-6 rounded-full bg-red-50 dark:bg-red-950/40 text-red-400 hover:bg-red-100 hover:text-red-600 dark:hover:text-red-400 text-xs flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">✕</button>
      ...
      {/* ── CONTEXT MENU ───────────────────────────────────────────────────── */}
      {contextMenu && (
        <div className="fixed z-[9999] bg-white dark:bg-slate-900 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 py-1.5 select-none min-w-[210px]" style={{ left: contextMenu.x, top: contextMenu.y }} onClick={(e) => e.stopPropagation()}>
          {contextMenu.targetId && (() => {
            ...
            return (
              <>
                <div className="px-3 py-1 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">{label}</div>
                ...
                <button onClick={() => { handleDeleteSelected(tid); closeContextMenu(); }} className="ctx-item text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/40">🗑️ Delete <span className="ml-auto text-slate-300">Del</span></button>
                <div className="h-px bg-slate-100 dark:bg-slate-800 mx-2 my-1" />
              </>
            );
          })()}
          <div className="px-3 py-1 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Add Element</div>
    ...
    <div className="flex h-full bg-[#1a1b22] text-white overflow-hidden select-none">
      ...
      <div className="w-64 shrink-0 flex flex-col bg-[#13141a] border-r border-white/[0.06] p-6 overflow-y-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-[11px] text-slate-500 dark:text-slate-400 pl-0.5">Card Designer</p>
        </div>

        {/* New section */}
        <div className="mb-6">
          <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.15em] mb-3">New</p>
          <div className="space-y-1.5">
            ...
            >
              ...
              <div>
                ...
                <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Photo card with staff data</div>
              </div>
            </button>
            <button
              onClick={onNewTPT}
            ...
            >
              ...
              <div>
                ...
                <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">Part-time teacher from timetable</div>
              </div>
            </button>
            {/* Certificate sub-section */}
            <div className="mt-1 mb-0.5">
              <p className="text-[9px] font-bold text-slate-600 dark:text-slate-300 uppercase tracking-widest mb-1.5 px-1">Certificates</p>
              <div className="space-y-1">
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[9px] text-slate-500 dark:text-slate-400 mt-0.5">Award for student records</div>
                  </div>
                </button>
                <button
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="text-[9px] text-slate-500 dark:text-slate-400 mt-0.5">Award for staff records</div>
                  </div>
                </button>
              </div>
            </div>
        ...
        {/* Open / Recent */}
        <div className="flex-1">
          <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.15em] mb-3">Open Recent</p>
          {savedTemplates.length > 0 ? (
                ...
                >
                  <div className="w-9 h-6 rounded bg-white/[0.07] flex items-center justify-center overflow-hidden shrink-0 border border-white/[0.05]">
                    {previews[tpl.id]
                      ...
                      : <span className="text-[8px] text-slate-600 dark:text-slate-300">…</span>}
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="text-[11px] font-medium text-slate-300 group-hover:text-white transition-colors truncate">{tpl.name}</div>
                    <div className="text-[9px] text-slate-600 dark:text-slate-300 capitalize">{tpl.cardType?.replace('-', ' ')}</div>
                  </div>
                </button>
              ))}
            </div>
            ...
            <div className="rounded-xl border border-white/[0.05] bg-white/[0.02] px-3 py-4 text-center">
              <p className="text-[11px] text-slate-600 dark:text-slate-300 leading-relaxed">No saved templates yet.<br />Use &ldquo;As Template&rdquo; in the editor to save one.</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Click a template to open it in the editor</p>
          </div>
          {!allLoaded && (
        ...
        {/* ID Card templates */}
        <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">🪪 ID Card Templates</h3>
          ...
          {BUILTIN_START_ITEMS.filter((i) => i.type === 'student' || i.type === 'staff' || i.type === 'teacher-part-time').map(({ key, design, label, emoji }) => (
            ...
            >
              ...
              <div className="px-2.5 py-2 border-t border-white/[0.05]">
                <div className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 group-hover:text-slate-200 transition-colors truncate">{emoji} {label}</div>
              </div>
            </button>
          ))}
        </div>
        ...
        <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3 mt-2">📜 Certificate Templates</h3>
        ...
        {savedTemplates.length > 0 && (
          <>
            <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-3">Your Saved Templates</h3>
            <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {savedTemplates.map((tpl) => (
                ...
                >
                  ...
                  <div className="px-2.5 py-2 border-t border-white/[0.05]">
                    <div className="text-[11px] font-semibold text-slate-400 dark:text-slate-500 group-hover:text-slate-200 truncate">{tpl.name}</div>
                    <div className="text-[9px] text-slate-600 dark:text-slate-300 capitalize mt-0.5">{tpl.cardType?.replace('-', ' ')}</div>
                  </div>
                </button>
              ))}
            </div>
