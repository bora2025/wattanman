  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between w-full px-6" style={{ maxWidth: '90vw' }}>
        ...
        <p className="text-slate-400 dark:text-slate-500 text-xs hidden sm:block">Drag the box or corner handles to crop</p>
        <button
          onClick={onClose}
          className="text-slate-400 dark:text-slate-500 hover:text-white transition-colors p-1 rounded"
          title="Close (Esc)"
        >
          ...
          <div className="absolute inset-0" style={{ pointerEvents: 'none', userSelect: 'none' }}>
            ...
            >
              ...
              {HANDLE_DEFS.map(({ id, style }) => (
                <div
                  key={id}
                  className="absolute bg-white dark:bg-slate-900 rounded-sm shadow-md"
                  style={{
                    ...style,
                    width: HANDLE_PX,
                    height: HANDLE_PX,
      ...
      <div className="flex items-center gap-3">
        {hasCrop && (
          <button
            ...
            className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-xs text-slate-400 dark:text-slate-500 hover:text-white transition-colors border border-slate-700"
          >
            Reset
          </button>
        )}
