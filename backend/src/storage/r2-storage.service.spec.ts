...
describe('R2StorageService', () => {
  ...
  it('sends a private SigV4-authenticated object upload', async () => {
    ...
    await service.putPrivate('quarantine/extensions/a package.zip', Buffer.from('zip'), 'application/zip');
  ...
  it('fails closed when R2 credentials are missing', async () => {
    ...
    await expect(new R2StorageService().putPrivate('test.zip', Buffer.from('zip'), 'application/zip')).rejects.toThrow(ServiceUnavailableException);
  });
  ...
  it('does not hide R2 upload failures', async () => {
    ...
    await expect(new R2StorageService().putPrivate('test.zip', Buffer.from('zip'), 'application/zip')).rejects.toThrow('R2 upload failed (503)');
  });

  it('signs private object deletion without sending a body', async () => {
    global.fetch = jest.fn().mockResolvedValue({ ok: true }) as any;

    await new R2StorageService().deletePrivate('quarantine/test.zip');
  ...
  it('downloads a private object through a signed request', async () => {
    ...
    const result = await new R2StorageService().getPrivate('validated/style.css');
