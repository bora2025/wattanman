...
async function main() {
  ...
  try {
    await prisma.$transaction(async (transaction) => {
      ...
      await transaction.$executeRawUnsafe('GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA public TO "wattaman_school_runtime"');
      ...
      for (const grantor of [owner, '"wattaman_migration"']) {
        ...
        await transaction.$executeRawUnsafe(`ALTER DEFAULT PRIVILEGES FOR ROLE ${grantor} IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO "wattaman_control_plane", "wattaman_school_runtime"`);
        await transaction.$executeRawUnsafe(`ALTER DEFAULT PRIVILEGES FOR ROLE ${grantor} IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO "wattaman_control_plane", "wattaman_school_runtime"`);
