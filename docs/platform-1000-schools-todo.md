...
- [ ] Add payment evidence upload through signed URLs.
  - Implementation is deployed: five-minute checksum-bound SigV4 PUT/GET URLs, authenticated object verification, exact request snapshots, and direct school/platform UI transfers passed a real private-R2 probe plus the 275-test suite. Production browser activation remains pending because the current object-only R2 token cannot apply bucket CORS (`PutBucketCors` returns `403 AccessDenied`). `backend/scripts/configure-r2-cors.js` now prefers Cloudflare's bearer-token management API and retains the S3 fallback; production currently has no `CLOUDFLARE_API_TOKEN`, so this gate remains open until a scoped bucket-management token applies and verifies the exact-origin policy.
...
- [x] Add school-scoped logical restore that cannot affect another tenant.
...
- [x] Classify core and extension data by sensitivity.
