...
| Stage 1 — Extension foundation | In progress | Versioned records, private R2 storage, bounded worker validation, review, immutable publication, installation, cleanup, audit, guarded migration bootstrap, failure handling, and an atomic legacy rollback flag exist; production-copy rehearsal and production backfill proof remain |
...
- [x] Run validation asynchronously with timeout, CPU, memory, and disk limits; validation runs in a terminable worker with bounded V8 heaps/stacks and performs no disk extraction.
...
- [x] Existing school modules and themes continue working unchanged.
...
- [x] The old direct CSS upload endpoint is retired after migration.
...
Migration bootstrap note (2026-08-03): `prisma/bootstrap-migrations.js` now creates the current schema only when PostgreSQL is completely empty, records every existing migration as the baseline, and then runs `prisma migrate deploy`. On an existing database without history it refuses by default; explicit `--adopt-existing` first requires a zero-difference `prisma migrate diff`, while unrelated drift is rejected without changing history. Railway startup passes this guarded adoption flag so a matching legacy database is baselined automatically. It also narrowly recognizes production being exactly one additive `ExtensionPilotFeedback` migration behind, baselines only the preceding migrations, and leaves the final table creation to `prisma migrate deploy`; any SQL outside that table still stops deployment. Fresh bootstrap, repeat deploy, exact-schema adoption, and mismatch refusal were rehearsed on PostgreSQL 16. Railway startup no longer uses `db push --accept-data-loss`; production-sized copy rehearsal remains open.
...
Validation-isolation note (2026-08-03): ZIP parsing, extraction, MIME detection, CSS rewriting, and manifest checks now execute in `extension-validation.worker.js`, never on the Nest request thread. The runner enforces a wall-clock deadline by terminating the worker, caps old/young V8 heaps and stack size, transfers only package bytes and validation metadata, and uses no filesystem extraction (disk allowance is zero). Worker tests prove successful asset-byte transfer, CPU-loop termination, and heap-exhaustion containment; the real PostgreSQL/R2 lifecycle publishes both module and theme ZIPs through the compiled worker. External publishers still require the separately deferred disposable-container and malware-scanning controls described in the threat model.
...
Backfill-verification note (2026-08-03): `db:backfill-extensions` now invokes a fail-closed row-level verifier, and `db:verify-extension-backfill` can be archived independently as deployment evidence. It compares every legacy catalog key, runtime/commercial mapping, active/listed state, and every school add-on's enabled and billing values against versioned records. A PostgreSQL 16 rehearsal with 80 mixed module/add-on/theme definitions across 12 schools (960 installation states) passed two identical backfill runs and an independent verification; deliberately flipping one installation was detected as an exact school/key mismatch. The production-copy checkbox remains open until the same commands run against an authorized production-sized restore.
