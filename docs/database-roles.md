...
- `wattaman_school_runtime`: CRUD without RLS bypass on the explicit tenant-table allowlist, plus read-only extension catalog access; used by school API and tenant workers.
...
Runtime table privileges are deny-by-default. Provisioning first revokes blanket table access, then grants only the reviewed tenant and catalog allowlists. Package assets and control-plane tables remain inaccessible, and default privileges do not expose newly created tables until they are deliberately reviewed.
